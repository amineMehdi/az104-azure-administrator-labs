...
Write-Host 'Lab: LAB-17'
Write-Host 'Location:' $Location
Write-Host 'Cost class: low'
Write-Host 'Role boundary: Network Contributor on the lab resource group'
$providers = @(
    'Microsoft.Network'
)
...
if ($rg -and $rg.Tags.purpose -eq 'az104-lab' -and $rg.Tags.labId -eq '17' -and $rg.Tags.runId -eq $RunId) {
...
$output = [ordered]@{ labId = 'LAB-17'; runId = $RunId; generatedAt = (Get-Date).ToUniversalTime().ToString('o'); result = $result; checks = @($checks) }
...
if (-not (Test-Path -LiteralPath $Manifest)) { throw "Missing state: $Manifest" }
$state = Get-Content -LiteralPath $Manifest -Raw | ConvertFrom-Json -Depth 20
Write-Host 'Cleanup preview for LAB-17'
...
if ($rg.ResourceId -ne $state.resourceGroup.id -or $rg.Tags.purpose -ne 'az104-lab' -or $rg.Tags.labId -ne '17' -or $rg.Tags.runId -ne $RunId) {
...
### Checkpoint 1: Create hub and spoke address spaces that do not overlap and subdivide them into purpose-specific subnets
...
### Checkpoint 2: Create both sides of VNet peering and interpret Connected, Initiated, and Disconnected states
...
### Checkpoint 3: Create a zone-redundant Standard static public IP where supported
...
### Checkpoint 4: Compare private address planning, public IP allocation, DNS labels, and effective peering routes
