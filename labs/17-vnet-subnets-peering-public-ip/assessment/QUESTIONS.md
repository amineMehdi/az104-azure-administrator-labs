...
## 1. Which statement best captures the key design principle for create vnets, subnets, peerings, and public ips? (`LAB17-Q01`)
...
## 2. You must begin the hands-on path for 'Create and configure virtual networks and subnets'. Which action matches the reviewed lab sequence? (`LAB17-Q02`)
...
## 3. Which authorization statement is appropriate before creating resource group, two virtual networks, four subnets in this lab? (`LAB17-Q03`)
...
- C. Confirm the active context and obtain only the declared role boundary: Network Contributor on the lab resource group
- D. Skip authorization checks because all resources use an AZ104 naming prefix.

## 4. The setup command for Lab 17 is run without its execution switch. What should happen? (`LAB17-Q04`)
...
## 5. Which validation evidence most directly proves the baseline resource boundary for Lab 17? (`LAB17-Q05`)

- A. Query the exact recorded scope and verify the intended resource/object types, including Microsoft.Network/virtualNetworks, Microsoft.Network/publicIPAddresses.
...
## 6. A learner reaches the externally gated checkpoint in Lab 17. What is the safest response? (`LAB17-Q06`)
...
## 7. Which command-side evidence best supports the first completed checkpoint in Lab 17? (`LAB17-Q07`)
...
- C. Run the independent validator and retain redacted structured output for the exact recorded scope after: Create hub and spoke address spaces that do not overlap and subdivide them into purpose-specific subnets.
- D. Use the setup process exit code alone without querying resource state.

## 8. During cleanup of Lab 17, several similarly named resources exist. Which targeting method is correct? (`LAB17-Q08`)
...
## 9. Setup completed for Lab 17, but one required state check fails. What is the best break/fix approach? (`LAB17-Q09`)
...
## 10. Which completion claim is valid immediately after this repository's offline tests pass for Lab 17? (`LAB17-Q10`)
