...
## 1. Which statement best captures the key design principle for control traffic with routes, nsgs, asgs, and effective rules? (`LAB18-Q01`)
...
## 2. You must begin the hands-on path for 'Configure user-defined routes'. Which action matches the reviewed lab sequence? (`LAB18-Q02`)
...
## 3. Which authorization statement is appropriate before creating resource group, virtual network, network security group in this lab? (`LAB18-Q03`)
...
## 4. The setup command for Lab 18 is run without its execution switch. What should happen? (`LAB18-Q04`)
...
## 5. Which validation evidence most directly proves the baseline resource boundary for Lab 18? (`LAB18-Q05`)

- A. Query the exact recorded scope and verify the intended resource/object types, including Microsoft.Network/networkSecurityGroups, Microsoft.Network/applicationSecurityGroups.
...
## 6. A learner reaches the externally gated checkpoint in Lab 18. What is the safest response? (`LAB18-Q06`)
...
## 7. Which command-side evidence best supports the first completed checkpoint in Lab 18? (`LAB18-Q07`)
...
- C. Run the independent validator and retain redacted structured output for the exact recorded scope after: Create frontend and backend ASGs and associate test NIC configurations.
- D. Use the setup process exit code alone without querying resource state.

## 8. During cleanup of Lab 18, several similarly named resources exist. Which targeting method is correct? (`LAB18-Q08`)
...
## 9. Setup completed for Lab 18, but one required state check fails. What is the best break/fix approach? (`LAB18-Q09`)
...
## 10. Which completion claim is valid immediately after this repository's offline tests pass for Lab 18? (`LAB18-Q10`)
