...
[Previous: Lab 22](../22-azure-monitor-logs-insights/README.md) · [Catalog](../README.md) · [Next: Lab 24](../24-azure-backup-restore/README.md)
...
Scenario: You are a monitoring administrator routing and suppressing actionable alerts. Build and prove this disposable service path: Storage metrics and activity events feed scoped alert rules, the action group provides the notification boundary, and a time-bounded processing rule suppresses actions only for the run-owned target.

Learner role: A monitoring administrator routing and suppressing actionable alerts.

Outcome: Use Azure CLI in PowerShell to create the monitored target and action group, create a scoped metric alert, and reconcile alert scope, criteria, actions, and suppression, with recoverable state and deterministic cleanup.
...
|---|---|---|
| `MR-MONITOR-04` | Set up alert rules, action groups, and alert processing rules in Azure Monitor | `LAB23-CP01`, `LAB23-CP02`, `LAB23-CP03`, `LAB23-CP04`, `LAB23-CP05` |
...
![Lab 23 service topology](diagrams/architecture.svg)

The editable source is [architecture.mmd](diagrams/architecture.mmd). Storage metrics and activity events feed scoped alert rules, the action group provides the notification boundary, and a time-bounded processing rule suppresses actions only for the run-owned target.

## Concept primer and design decisions

Alert rules evaluate signals; action groups deliver notifications; the common alert schema standardizes payloads; processing rules suppress or add actions without preventing alert evaluation.
...
|---|:---:|---|---|---|
| `run-id` — Unique lowercase run ownership identifier | Yes | parameter | `az104l23-01` | `block` |
...
| `alert-email` — Approved action-group receiver | No | environment (`AZ104_ALERT_EMAIL`) | `alerts@example.test` | `skip-checkpoint` |
...
./scripts/cli/Preflight.ps1 -SubscriptionId $env:AZ104_SUBSCRIPTION_ID -RunId 'az104l23-01' -Location 'westeurope'
```
...
## Task 1 — Verify metric, action-group, and processing-rule support {#task-1}

Checkpoint: `LAB23-CP01`

Purpose and operational relevance: Read the provider, region, SKU, feature, quota, and companion-tool signals needed for verify metric, action-group, and processing-rule support, without changing Azure state. The change is unsafe when alert creation fails when the metric name is valid for a different resource type or namespace. This checkpoint therefore proves: The storage metric namespace exposes Transactions and the subscription can enumerate action groups and alert-processing rules.
...
Expected state: The storage metric namespace exposes Transactions and the subscription can enumerate action groups and alert-processing rules.

Representative redacted output:

```text
The storage metric namespace exposes Transactions and the subscription can enumerate action groups and alert-processing rules.
```
...
Expected positive result: The storage metric namespace exposes Transactions and the subscription can enumerate action groups and alert-processing rules.
...
Expected negative result: LAB23-CP01 negative boundary: The undesired state is absent; the negative assertion must not report: Microsoft.Storage is not registered.
...
- Positive assertion proving: The storage metric namespace exposes Transactions and the subscription can enumerate action groups and alert-processing rules.
- Negative assertion proving absence of: Microsoft.Storage is not registered.

Common failure and safe retry: Alert creation fails when the metric name is valid for a different resource type or namespace. Retry LAB23-CP01 for verify metric, action-group, and processing-rule support only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After LAB23-CP02, LAB23-CP03, LAB23-CP04, LAB23-CP05, remove the exact manifest IDs created or configured by LAB23-CP01 for verify metric, action-group, and processing-rule support; its residual probe must then return zero active IDs.

## Task 2 — Create the monitored target and action group {#task-2}

Checkpoint: `LAB23-CP02`

Purpose and operational relevance: Provision and immediately inventory the exact run-owned resources needed to create the monitored target and action group, so partial completion remains recoverable. The change is unsafe when an invalid receiver address or unsupported receiver type can block the action-group deployment. This checkpoint therefore proves: The storage target and disabled test notification receiver are run-owned, with no unsolicited message sent during setup.
...
Expected state: The storage target and disabled test notification receiver are run-owned, with no unsolicited message sent during setup.

Representative redacted output:

```text
The storage target and disabled test notification receiver are run-owned, with no unsolicited message sent during setup.
```

Positive validation:

```powershell
$metricEnabled = az monitor metrics alert show --resource-group $ResourceGroupName --name $alert --query enabled --output tsv
if ($metricEnabled -ne 'true') { throw 'The metric alert is disabled.' }
```

Expected positive result: The storage target and disabled test notification receiver are run-owned, with no unsolicited message sent during setup.

Negative validation:

```powershell
$activityEnabled = az monitor activity-log alert show --resource-group $ResourceGroupName --name "activity-$suffix" --query enabled --output tsv
if ($activityEnabled -ne 'true') { throw 'The activity log alert is disabled.' }
```

Expected negative result: LAB23-CP02 negative boundary: The undesired state is absent; the negative assertion must not report: The activity log alert is disabled.
...
- Positive assertion proving: The storage target and disabled test notification receiver are run-owned, with no unsolicited message sent during setup.
- Negative assertion proving absence of: The activity log alert is disabled.

Common failure and safe retry: An invalid receiver address or unsupported receiver type can block the action-group deployment. Retry LAB23-CP02 for create the monitored target and action group only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After LAB23-CP03, LAB23-CP04, LAB23-CP05, remove the exact manifest IDs created or configured by LAB23-CP02 for create the monitored target and action group; its residual probe must then return zero active IDs.

## Task 3 — Create a scoped metric alert {#task-3}

Checkpoint: `LAB23-CP03`

Purpose and operational relevance: Configure and independently inspect the control-plane and data-path properties needed to create a scoped metric alert. The change is unsafe when a resource ID assigned only during setup must be reconstructed in validation or the alert scope check fails under strict mode. This checkpoint therefore proves: The alert rule targets only the run-owned storage account, uses the intended aggregation/threshold, and references the exact action-group ID.
...
Expected state: The alert rule targets only the run-owned storage account, uses the intended aggregation/threshold, and references the exact action-group ID.

Representative redacted output:

```text
The alert rule targets only the run-owned storage account, uses the intended aggregation/threshold, and references the exact action-group ID.
```

Positive validation:

```powershell
$ruleType = az monitor alert-processing-rule show --resource-group $ResourceGroupName --name $processingRule --query properties.ruleType --output tsv
if ($ruleType -ne 'RemoveAllActionGroups') { throw "Processing rule type is $ruleType." }
```

Expected positive result: The alert rule targets only the run-owned storage account, uses the intended aggregation/threshold, and references the exact action-group ID.

Negative validation:

```powershell
$scope = az monitor alert-processing-rule show --resource-group $ResourceGroupName --name $processingRule --query "properties.scopes[0]" --output tsv
if ($scope -ne $storageId) { throw 'The processing rule scope is incorrect.' }
```

Expected negative result: LAB23-CP03 negative boundary: The undesired state is absent; the negative assertion must not report: The processing rule scope is incorrect.
...
- Positive assertion proving: The alert rule targets only the run-owned storage account, uses the intended aggregation/threshold, and references the exact action-group ID.
- Negative assertion proving absence of: The processing rule scope is incorrect.

Common failure and safe retry: A resource ID assigned only during setup must be reconstructed in validation or the alert scope check fails under strict mode. Retry LAB23-CP03 for create a scoped metric alert only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After LAB23-CP04, LAB23-CP05, remove the exact manifest IDs created or configured by LAB23-CP03 for create a scoped metric alert; its residual probe must then return zero active IDs.

## Task 4 — Apply a bounded alert-processing suppression rule {#task-4}

Checkpoint: `LAB23-CP04`

Purpose and operational relevance: Exercise the deterministic operational or denied path needed to apply a bounded alert-processing suppression rule, then retain comparable before-and-after results. The change is unsafe when a subscription-wide scope or open-ended schedule can suppress operational alerts beyond the lab target. This checkpoint therefore proves: The processing rule is enabled only for the declared scope and schedule, without muting unrelated subscription alerts.
...
Expected state: The processing rule is enabled only for the declared scope and schedule, without muting unrelated subscription alerts.

Representative redacted output:

```text
The processing rule is enabled only for the declared scope and schedule, without muting unrelated subscription alerts.
```

Positive validation:

```powershell
$receivers = az monitor action-group show --resource-group $ResourceGroupName --name $actionGroup --query "emailReceivers[?name=='learner'] | length(@)" --output tsv
if ([int]$receivers -ne 1) { throw 'The optional email receiver is absent.' }
```

Expected positive result: The processing rule is enabled only for the declared scope and schedule, without muting unrelated subscription alerts.

Negative validation:

```powershell
$disabledReceivers = az monitor action-group show --resource-group $ResourceGroupName --name $actionGroup --query "emailReceivers[?status=='Disabled'] | length(@)" --output tsv
if ([int]$disabledReceivers -ne 0) { throw 'An action-group email receiver is disabled.' }
```

Expected negative result: LAB23-CP04 negative boundary: The undesired state is absent; the negative assertion must not report: An action-group email receiver is disabled.
...
- Positive assertion proving: The processing rule is enabled only for the declared scope and schedule, without muting unrelated subscription alerts.
- Negative assertion proving absence of: An action-group email receiver is disabled.

Common failure and safe retry: A subscription-wide scope or open-ended schedule can suppress operational alerts beyond the lab target. Retry LAB23-CP04 for apply a bounded alert-processing suppression rule only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After LAB23-CP05, remove the exact manifest IDs created or configured by LAB23-CP04 for apply a bounded alert-processing suppression rule; its residual probe must then return zero active IDs.

## Task 5 — Reconcile alert scope, criteria, actions, and suppression {#task-5}

Checkpoint: `LAB23-CP05`

Purpose and operational relevance: Reconcile live service properties, returned IDs, and dependency-aware removal needed to reconcile alert scope, criteria, actions, and suppression before handoff. The change is unsafe when seeing an enabled alert does not prove its action or processing rule is scoped correctly. This checkpoint therefore proves: Alert criteria, target ID, action-group binding, and processing-rule scope all resolve to the run-owned resources.
...
Expected state: Alert criteria, target ID, action-group binding, and processing-rule scope all resolve to the run-owned resources.

Representative redacted output:

```text
Alert criteria, target ID, action-group binding, and processing-rule scope all resolve to the run-owned resources.
```

Positive validation:

```powershell
$actionCount = az monitor metrics alert show --resource-group $ResourceGroupName --name $alert --query "actions | length(@)" --output tsv
if ([int]$actionCount -ne 1) { throw "Metric alert action count is $actionCount." }
```

Expected positive result: Alert criteria, target ID, action-group binding, and processing-rule scope all resolve to the run-owned resources.

Negative validation:

```powershell
$wrongScope = az monitor metrics alert show --resource-group $ResourceGroupName --name $alert --query "scopes[?@!='$storageId'] | length(@)" --output tsv
if ([int]$wrongScope -ne 0) { throw 'The metric alert has an unexpected scope.' }
```

Expected negative result: LAB23-CP05 negative boundary: The undesired state is absent; the negative assertion must not report: The metric alert has an unexpected scope.
...
- Positive assertion proving: Alert criteria, target ID, action-group binding, and processing-rule scope all resolve to the run-owned resources.
- Negative assertion proving absence of: The metric alert has an unexpected scope.

Common failure and safe retry: Seeing an enabled alert does not prove its action or processing rule is scoped correctly. Retry LAB23-CP05 for reconcile alert scope, criteria, actions, and suppression only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After all service validation and evidence capture, remove the exact manifest IDs created or configured by LAB23-CP05 for reconcile alert scope, criteria, actions, and suppression; its residual probe must then return zero active IDs.
...
```powershell
./scripts/cli/Validate.ps1 -RunId 'az104l23-01' -Mode Deployment
az resource list --resource-group 'rg-az104-l23-az104l23-01' --query '[].{name:name,type:type}' --output table
```
...
```powershell
az monitor metrics alert update --resource-group $ResourceGroupName --name $alert --enabled false --output none
```

Capture the failed state with a read-only query:

```powershell
az monitor metrics alert show --resource-group $ResourceGroupName --name $alert --query "{enabled:enabled,scopes:scopes,actions:actions}" --output json
```

Repair only the injected setting, then repeat the same query:

```powershell
az monitor metrics alert update --resource-group $ResourceGroupName --name $alert --enabled true --output none
az monitor metrics alert show --resource-group $ResourceGroupName --name $alert --query enabled --output tsv
```

Expected symptom: The alert rule remains visible but no longer evaluates the capacity signal.
...
Design alert routing for business hours, after-hours escalation, and planned maintenance.
...
```powershell
./scripts/cli/Cleanup.ps1 -RunId 'az104l23-01'
./scripts/cli/Cleanup.ps1 -RunId 'az104l23-01' -Execute
./scripts/cli/Validate.ps1 -RunId 'az104l23-01' -Mode PostCleanup
$wrongScope = az monitor metrics alert show --resource-group $ResourceGroupName --name $alert --query "scopes[?@!='$storageId'] | length(@)" --output tsv
if ([int]$wrongScope -ne 0) { throw 'The metric alert has an unexpected scope.' }
```
...
- [Create an Azure Monitor metric alert](https://learn.microsoft.com/en-us/azure/azure-monitor/alerts/alerts-create-metric-alert-rule)
- [Create Azure Monitor activity log alerts](https://learn.microsoft.com/en-us/azure/azure-monitor/alerts/alerts-create-activity-log-alert-rule)
...
$gatePresent = -not [string]::IsNullOrWhiteSpace([Environment]::GetEnvironmentVariable('AZ104_ALERT_EMAIL'))
Add-PreflightResult -Id 'optional-gate:AZ104_ALERT_EMAIL' -Required $false -Passed $gatePresent -Actual $(if ($gatePresent) { 'present (value redacted)' } else { 'absent; checkpoint will be skipped' })
...
if ($state.labId -ne 'LAB-23' -or $state.runId -ne $RunId) { throw 'Manifest ownership does not match this lab and run ID.' }
$PSNativeCommandUseErrorActionPreference = $false
...
$ResourceGroupName = $(if ('23' -in @('00', '01', '02')) { $null } else { "rg-az104-l23-$RunId" })
$suffix = (($RunId -replace '[^a-z0-9]', '') + '000000000000').Substring(0, 12)
$storage = "st23$suffix"; $actionGroup = "ag-$suffix"; $alert = "alert-$suffix"; $processingRule = "maintenance-$suffix"; $storageId = az storage account show --resource-group $ResourceGroupName --name $storage --query id --output tsv
$actionId = az monitor action-group show --resource-group $ResourceGroupName --name $actionGroup --query id --output tsv
...
# CHECKPOINT LAB23-CP01 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB23-CP01' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP01' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab23-cp01.positive' -CheckpointId 'LAB23-CP01' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab23-cp01.negative' -CheckpointId 'LAB23-CP01' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
...
if ($insights -ne 'Registered') { throw 'Microsoft.Insights is not registered.' }
    ...
    Add-ValidationCheck -Id 'lab23-cp01.positive' -CheckpointId 'LAB23-CP01' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $positivePassed -Command 'query each manifest-recorded object by exact ID and run the authored service probe' -Expected 'Checkpoint state is pass, recorded objects exist, and service state matches.' -Actual $positiveActual
...
if ($storageProvider -ne 'Registered') { throw 'Microsoft.Storage is not registered.' }
    ...
    Add-ValidationCheck -Id 'lab23-cp01.negative' -CheckpointId 'LAB23-CP01' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    ...
    Add-ValidationCheck -Id 'lab23-cp01.residual' -CheckpointId 'LAB23-CP01' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB23-CP01 END

# CHECKPOINT LAB23-CP02 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB23-CP02' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP02' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab23-cp02.positive' -CheckpointId 'LAB23-CP02' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab23-cp02.negative' -CheckpointId 'LAB23-CP02' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
    $positivePassed = $checkpointState.status -eq 'pass'
    $positiveActual = "checkpoint=$($checkpointState.status); managedObjects=$($checkpointObjects.Count)"
    foreach ($managedObject in $checkpointObjects) {
...
if ($metricEnabled -ne 'true') { throw 'The metric alert is disabled.' }
    ...
    Add-ValidationCheck -Id 'lab23-cp02.positive' -CheckpointId 'LAB23-CP02' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $positivePassed -Command 'query each manifest-recorded object by exact ID and run the authored service probe' -Expected 'Checkpoint state is pass, recorded objects exist, and service state matches.' -Actual $positiveActual
...
if ($activityEnabled -ne 'true') { throw 'The activity log alert is disabled.' }
    ...
    $negativePassed = $unsafe.Count -eq 0 -and $negativeProbePassed
    $negativeActual = "unsafeObjects=$($unsafe.Count)" + "; $negativeProbeActual"
    Add-ValidationCheck -Id 'lab23-cp02.negative' -CheckpointId 'LAB23-CP02' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    $active = [System.Collections.Generic.List[string]]::new()
    foreach ($managedObject in $checkpointObjects) {
    ...
    Add-ValidationCheck -Id 'lab23-cp02.residual' -CheckpointId 'LAB23-CP02' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB23-CP02 END

# CHECKPOINT LAB23-CP03 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB23-CP03' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP03' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab23-cp03.positive' -CheckpointId 'LAB23-CP03' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab23-cp03.negative' -CheckpointId 'LAB23-CP03' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
    $positivePassed = $checkpointState.status -eq 'pass'
    $positiveActual = "checkpoint=$($checkpointState.status); managedObjects=$($checkpointObjects.Count)"
    foreach ($managedObject in $checkpointObjects) {
...
if ($ruleType -ne 'RemoveAllActionGroups') { throw "Processing rule type is $ruleType." }
    ...
    Add-ValidationCheck -Id 'lab23-cp03.positive' -CheckpointId 'LAB23-CP03' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $positivePassed -Command 'query each manifest-recorded object by exact ID and run the authored service probe' -Expected 'Checkpoint state is pass, recorded objects exist, and service state matches.' -Actual $positiveActual
...
if ($scope -ne $storageId) { throw 'The processing rule scope is incorrect.' }
    ...
    $negativePassed = $unsafe.Count -eq 0 -and $negativeProbePassed
    $negativeActual = "unsafeObjects=$($unsafe.Count)" + "; $negativeProbeActual"
    Add-ValidationCheck -Id 'lab23-cp03.negative' -CheckpointId 'LAB23-CP03' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    $active = [System.Collections.Generic.List[string]]::new()
    foreach ($managedObject in $checkpointObjects) {
    ...
    Add-ValidationCheck -Id 'lab23-cp03.residual' -CheckpointId 'LAB23-CP03' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB23-CP03 END

# CHECKPOINT LAB23-CP04 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB23-CP04' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP04' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab23-cp04.positive' -CheckpointId 'LAB23-CP04' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab23-cp04.negative' -CheckpointId 'LAB23-CP04' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
    $positivePassed = $checkpointState.status -eq 'pass'
    $positiveActual = "checkpoint=$($checkpointState.status); managedObjects=$($checkpointObjects.Count)"
    foreach ($managedObject in $checkpointObjects) {
...
if ([int]$receivers -ne 1) { throw 'The optional email receiver is absent.' }
    ...
    Add-ValidationCheck -Id 'lab23-cp04.positive' -CheckpointId 'LAB23-CP04' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $positivePassed -Command 'query each manifest-recorded object by exact ID and run the authored service probe' -Expected 'Checkpoint state is pass, recorded objects exist, and service state matches.' -Actual $positiveActual
...
if ([int]$disabledReceivers -ne 0) { throw 'An action-group email receiver is disabled.' }
    ...
    $negativePassed = $unsafe.Count -eq 0 -and $negativeProbePassed
    $negativeActual = "unsafeObjects=$($unsafe.Count)" + "; $negativeProbeActual"
    Add-ValidationCheck -Id 'lab23-cp04.negative' -CheckpointId 'LAB23-CP04' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    $active = [System.Collections.Generic.List[string]]::new()
    foreach ($managedObject in $checkpointObjects) {
    ...
    Add-ValidationCheck -Id 'lab23-cp04.residual' -CheckpointId 'LAB23-CP04' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB23-CP04 END

# CHECKPOINT LAB23-CP05 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB23-CP05' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP05' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab23-cp05.positive' -CheckpointId 'LAB23-CP05' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab23-cp05.negative' -CheckpointId 'LAB23-CP05' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
    $positivePassed = $checkpointState.status -eq 'pass'
    $positiveActual = "checkpoint=$($checkpointState.status); managedObjects=$($checkpointObjects.Count)"
    foreach ($managedObject in $checkpointObjects) {
...
if ([int]$wrongScope -ne 0) { throw 'The metric alert has an unexpected scope.' }
    ...
    $negativePassed = $unsafe.Count -eq 0 -and $negativeProbePassed
    $negativeActual = "unsafeObjects=$($unsafe.Count)" + "; $negativeProbeActual"
    Add-ValidationCheck -Id 'lab23-cp05.negative' -CheckpointId 'LAB23-CP05' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    $active = [System.Collections.Generic.List[string]]::new()
    foreach ($managedObject in $checkpointObjects) {
    ...
    Add-ValidationCheck -Id 'lab23-cp05.residual' -CheckpointId 'LAB23-CP05' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB23-CP05 END
...
function Write-CleanupRefusal {
    ...
    $refusal = @{
        schemaVersion = '1.0.0'; labId = 'LAB-23'; runId = $RunId
        generatedAt = (Get-Date).ToUniversalTime().ToString('o')
...
if ($state.labId -ne 'LAB-23' -or $state.runId -ne $RunId) {
...
foreach ($object in $active) {
    ...
    if ($method -eq 'manifest-id-and-tags' -and $object.kind -eq 'azure-resource') {
        ...
        $manifestTagsMatch =
            ...
            [string]$expectedTags['labId'] -eq '23' -and
...
# CHECKPOINT LAB23-CP05 BEGIN
$targets = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP05' -and $_.lifecycleStatus -eq 'active' })
foreach ($target in $targets) {
    $alreadyAbsent = $absentIds.Contains([string]$target.id)
    $actions.Add(@{ checkpointId = 'LAB23-CP05'; targetId = [string]$target.id; targetType = [string]$target.type; ownership = @{ method = [string]$target.ownership.method; verified = [bool]$verifiedOwnershipById[[string]$target.id] }; status = $(if ($alreadyAbsent) { 'skipped' } else { 'preview' }); message = $(if ($alreadyAbsent) { 'The exact manifest ID was already absent; idempotent cleanup recorded the safe end state.' } elseif ($Execute) { 'Pending removal through the dependency-safe ownership boundary.' } else { 'Would remove this exact manifest-recorded target.' }) })
}
$state.updatedAt = (Get-Date).ToUniversalTime().ToString('o')
$state | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Manifest -Encoding utf8
# CHECKPOINT LAB23-CP05 END
...
# CHECKPOINT LAB23-CP04 BEGIN
$targets = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP04' -and $_.lifecycleStatus -eq 'active' })
foreach ($target in $targets) {
    $alreadyAbsent = $absentIds.Contains([string]$target.id)
    $actions.Add(@{ checkpointId = 'LAB23-CP04'; targetId = [string]$target.id; targetType = [string]$target.type; ownership = @{ method = [string]$target.ownership.method; verified = [bool]$verifiedOwnershipById[[string]$target.id] }; status = $(if ($alreadyAbsent) { 'skipped' } else { 'preview' }); message = $(if ($alreadyAbsent) { 'The exact manifest ID was already absent; idempotent cleanup recorded the safe end state.' } elseif ($Execute) { 'Pending removal through the dependency-safe ownership boundary.' } else { 'Would remove this exact manifest-recorded target.' }) })
}
$state.updatedAt = (Get-Date).ToUniversalTime().ToString('o')
$state | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Manifest -Encoding utf8
# CHECKPOINT LAB23-CP04 END
...
# CHECKPOINT LAB23-CP03 BEGIN
$targets = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP03' -and $_.lifecycleStatus -eq 'active' })
foreach ($target in $targets) {
    $alreadyAbsent = $absentIds.Contains([string]$target.id)
    $actions.Add(@{ checkpointId = 'LAB23-CP03'; targetId = [string]$target.id; targetType = [string]$target.type; ownership = @{ method = [string]$target.ownership.method; verified = [bool]$verifiedOwnershipById[[string]$target.id] }; status = $(if ($alreadyAbsent) { 'skipped' } else { 'preview' }); message = $(if ($alreadyAbsent) { 'The exact manifest ID was already absent; idempotent cleanup recorded the safe end state.' } elseif ($Execute) { 'Pending removal through the dependency-safe ownership boundary.' } else { 'Would remove this exact manifest-recorded target.' }) })
}
$state.updatedAt = (Get-Date).ToUniversalTime().ToString('o')
$state | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Manifest -Encoding utf8
# CHECKPOINT LAB23-CP03 END
...
# CHECKPOINT LAB23-CP02 BEGIN
$targets = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP02' -and $_.lifecycleStatus -eq 'active' })
foreach ($target in $targets) {
    $alreadyAbsent = $absentIds.Contains([string]$target.id)
    $actions.Add(@{ checkpointId = 'LAB23-CP02'; targetId = [string]$target.id; targetType = [string]$target.type; ownership = @{ method = [string]$target.ownership.method; verified = [bool]$verifiedOwnershipById[[string]$target.id] }; status = $(if ($alreadyAbsent) { 'skipped' } else { 'preview' }); message = $(if ($alreadyAbsent) { 'The exact manifest ID was already absent; idempotent cleanup recorded the safe end state.' } elseif ($Execute) { 'Pending removal through the dependency-safe ownership boundary.' } else { 'Would remove this exact manifest-recorded target.' }) })
}
$state.updatedAt = (Get-Date).ToUniversalTime().ToString('o')
$state | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Manifest -Encoding utf8
# CHECKPOINT LAB23-CP02 END
...
# CHECKPOINT LAB23-CP01 BEGIN
$targets = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP01' -and $_.lifecycleStatus -eq 'active' })
foreach ($target in $targets) {
    $alreadyAbsent = $absentIds.Contains([string]$target.id)
    $actions.Add(@{ checkpointId = 'LAB23-CP01'; targetId = [string]$target.id; targetType = [string]$target.type; ownership = @{ method = [string]$target.ownership.method; verified = [bool]$verifiedOwnershipById[[string]$target.id] }; status = $(if ($alreadyAbsent) { 'skipped' } else { 'preview' }); message = $(if ($alreadyAbsent) { 'The exact manifest ID was already absent; idempotent cleanup recorded the safe end state.' } elseif ($Execute) { 'Pending removal through the dependency-safe ownership boundary.' } else { 'Would remove this exact manifest-recorded target.' }) })
}
$state.updatedAt = (Get-Date).ToUniversalTime().ToString('o')
$state | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Manifest -Encoding utf8
# CHECKPOINT LAB23-CP01 END
...
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP01' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp01.residual'; command = 'query manifest-recorded IDs for LAB23-CP01'; expected = 'zero active objects for LAB23-CP01'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP02' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp02.residual'; command = 'query manifest-recorded IDs for LAB23-CP02'; expected = 'zero active objects for LAB23-CP02'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP03' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp03.residual'; command = 'query manifest-recorded IDs for LAB23-CP03'; expected = 'zero active objects for LAB23-CP03'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP04' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp04.residual'; command = 'query manifest-recorded IDs for LAB23-CP04'; expected = 'zero active objects for LAB23-CP04'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB23-CP05' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp05.residual'; command = 'query manifest-recorded IDs for LAB23-CP05'; expected = 'zero active objects for LAB23-CP05'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
