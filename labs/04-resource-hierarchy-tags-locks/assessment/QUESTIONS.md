...
## 1. Which statement best captures the key design principle for manage hierarchy, resource groups, tags, and locks? (`LAB04-Q01`)
...
## 2. You must begin the hands-on path for 'Configure resource locks'. Which action matches the reviewed lab sequence? (`LAB04-Q02`)
...
## 3. Which authorization statement is appropriate before creating resource group, resource lock, optional management group in this lab? (`LAB04-Q03`)
...
- C. Confirm the active context and obtain only the declared role boundary: Contributor plus User Access Administrator for lock management; Management Group Contributor for the optional hierarchy path
- D. Skip authorization checks because all resources use an AZ104 naming prefix.

## 4. The setup command for Lab 04 is run without its execution switch. What should happen? (`LAB04-Q04`)
...
## 5. Which validation evidence most directly proves the baseline resource boundary for Lab 04? (`LAB04-Q05`)

- A. Query the exact recorded scope and verify the intended resource/object types, including Microsoft.Authorization/locks.
...
## 6. A learner reaches the externally gated checkpoint in Lab 04. What is the safest response? (`LAB04-Q06`)

- A. Invent a placeholder tenant, email address, domain, or production target and continue.
- B. Stop that branch unless its prerequisites are explicitly satisfied: Management-group creation is optional and requires explicit tenant hierarchy authorization.
...
## 7. Which command-side evidence best supports the first completed checkpoint in Lab 04? (`LAB04-Q07`)
...
- C. Run the independent validator and retain redacted structured output for the exact recorded scope after: Create a resource group with purpose, labId, runId, owner, and expiresOn tags.
- D. Use the setup process exit code alone without querying resource state.

## 8. During cleanup of Lab 04, several similarly named resources exist. Which targeting method is correct? (`LAB04-Q08`)
...
## 9. Setup completed for Lab 04, but one required state check fails. What is the best break/fix approach? (`LAB04-Q09`)
...
## 10. Which completion claim is valid immediately after this repository's offline tests pass for Lab 04? (`LAB04-Q10`)
