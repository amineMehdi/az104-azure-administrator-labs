...
Write-Host 'Lab: LAB-11'
Write-Host "Tenant: $($account.tenantId)"
Write-Host "Subscription: $SubscriptionId"
Write-Host "Location: $Location"
Write-Host 'Cost class: moderate'
Write-Host 'Role boundary: Virtual Machine Contributor and Network Contributor on the lab resource group'
...
$ResourceGroupName = "rg-az104-l11-$RunId"
$suffix = (($RunId -replace '[^a-z0-9]', '') + '000000000000').Substring(0, 12)

Write-Host 'LAB-11 plan'
Write-Host "  subscription: $SubscriptionId"
Write-Host "  location: $Location"
Write-Host '  scope: subscription'
Write-Host '  cost class: moderate'
Write-Host '  resources: resource group, virtual network, network security group, network interface, Linux virtual machine, managed data disk'
if (-not $Execute) {
...
if ('subscription' -eq 'local') {
    ...
    if ($resourceGroup.tags.purpose -eq 'az104-lab' -and $resourceGroup.tags.labId -eq '11' -and $resourceGroup.tags.runId -eq $RunId) {
...
$output = [ordered]@{ labId = 'LAB-11'; runId = $RunId; generatedAt = (Get-Date).ToUniversalTime().ToString('o'); result = $result; checks = @($checks) }
...
if (-not $SubscriptionId) { $SubscriptionId = [string]$account.id }
if ($account.id -ne $SubscriptionId -or $state.subscriptionId -ne $SubscriptionId) { throw 'Active or recorded subscription mismatch.' }
Write-Host 'Cleanup preview for LAB-11'
...
if ($resourceGroup.id -ne $state.resourceGroup.id -or $resourceGroup.tags.purpose -ne 'az104-lab' -or $resourceGroup.tags.labId -ne '11' -or $resourceGroup.tags.runId -ne $RunId) {
...
```powershell
pwsh ./scripts/cli/Setup.ps1 -RunId az104l11-01 -SubscriptionId <subscription-id> -Location <region>
```
...
```powershell
pwsh ./scripts/cli/Setup.ps1 -RunId az104l11-01 -SubscriptionId <subscription-id> -Location <region> -Execute
```
...
```powershell
pwsh ./scripts/cli/Validate.ps1 -RunId az104l11-01 -SubscriptionId <subscription-id>
```
...
```powershell
pwsh ./scripts/cli/Cleanup.ps1 -RunId az104l11-01 -SubscriptionId <subscription-id>
```

After verifying every printed target belongs to this run:

```powershell
pwsh ./scripts/cli/Cleanup.ps1 -RunId az104l11-01 -SubscriptionId <subscription-id> -Execute
```
