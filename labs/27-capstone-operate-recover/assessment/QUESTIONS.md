...
## 1. Which statement best captures the key design principle for capstone — operate, troubleshoot, back up, and recover an azure workload? (`LAB27-Q01`)

- A. Operations should follow evidence: establish baseline, detect and scope the fault, repair the smallest cause, validate service state, then prove backup or recovery objectives before cleanup.
...
## 2. You must begin the hands-on path for 'Interpret access assignments'. Which action matches the reviewed lab sequence? (`LAB27-Q02`)

- A. Delete similarly named resources before recording the current subscription.
- B. Inventory the workload, interpret direct/inherited access, policy compliance, tags, locks, and current health before changing anything.
...
## 3. Which authorization statement is appropriate before creating resource group, test workload, Log Analytics workspace in this lab? (`LAB27-Q03`)
...
- C. Confirm the active context and obtain only the declared role boundary: Contributor, Monitoring Contributor, Backup Contributor, and Network Contributor on the capstone resource group
- D. Skip authorization checks because all resources use an AZ104 naming prefix.

## 4. The setup command for Lab 27 is run without its execution switch. What should happen? (`LAB27-Q04`)
...
## 5. Which validation evidence most directly proves the baseline resource boundary for Lab 27? (`LAB27-Q05`)

- A. Query the exact recorded scope and verify the intended resource/object types, including Microsoft.Compute/virtualMachines, Microsoft.OperationalInsights/workspaces.
...
## 6. A learner reaches the externally gated checkpoint in Lab 27. What is the safest response? (`LAB27-Q06`)

- A. Invent a placeholder tenant, email address, domain, or production target and continue.
- B. Stop that branch unless its prerequisites are explicitly satisfied: The live recovery drill requires cost review and an isolated workload; production failover or irreversible deletion is never inferred from this capstone.
...
## 7. Which command-side evidence best supports the first completed checkpoint in Lab 27? (`LAB27-Q07`)
...
- C. Run the independent validator and retain redacted structured output for the exact recorded scope after: Inventory the workload, interpret direct/inherited access, policy compliance, tags, locks, and current health before changing anything.
- D. Use the setup process exit code alone without querying resource state.

## 8. During cleanup of Lab 27, several similarly named resources exist. Which targeting method is correct? (`LAB27-Q08`)
...
## 9. Setup completed for Lab 27, but one required state check fails. What is the best break/fix approach? (`LAB27-Q09`)
...
## 10. Which completion claim is valid immediately after this repository's offline tests pass for Lab 27? (`LAB27-Q10`)
