...
if ('subscription' -eq 'local') {
    ...
    if ($resourceGroup.tags.purpose -eq 'az104-lab' -and $resourceGroup.tags.labId -eq '27' -and $resourceGroup.tags.runId -eq $RunId) {
...
if ($resourceGroup.id -ne $state.resourceGroup.id -or $resourceGroup.tags.purpose -ne 'az104-lab' -or $resourceGroup.tags.labId -ne '27' -or $resourceGroup.tags.runId -ne $RunId) {
...
```ps1
pwsh ./scripts/cli/Setup.ps1 -RunId az104l27-01 -SubscriptionId <subscription-id> -Location <region>
```
...
```ps1
pwsh ./scripts/cli/Setup.ps1 -RunId az104l27-01 -SubscriptionId <subscription-id> -Location <region> -Execute
```
...
```ps1
pwsh ./scripts/cli/Validate.ps1 -RunId az104l27-01 -SubscriptionId <subscription-id>
```
...
```ps1
pwsh ./scripts/cli/Cleanup.ps1 -RunId az104l27-01 -SubscriptionId <subscription-id>
```

After verifying every printed target belongs to this run:

```ps1
pwsh ./scripts/cli/Cleanup.ps1 -RunId az104l27-01 -SubscriptionId <subscription-id> -Execute
```
