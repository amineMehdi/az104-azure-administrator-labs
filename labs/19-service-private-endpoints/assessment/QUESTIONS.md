...
## 1. Which statement best captures the key design principle for compare service endpoints and private endpoints? (`LAB19-Q01`)

- A. Service endpoints keep the service's public endpoint while adding subnet identity, whereas private endpoints place a private NIC in the VNet and depend on correct private DNS resolution.
...
## 2. You must begin the hands-on path for 'Configure Azure Storage firewalls and virtual networks'. Which action matches the reviewed lab sequence? (`LAB19-Q02`)

- A. Delete similarly named resources before recording the current subscription.
- B. Create separate service-endpoint and private-endpoint subnets with appropriate policies.
...
## 3. Which authorization statement is appropriate before creating resource group, virtual network, service-endpoint subnet in this lab? (`LAB19-Q03`)
...
- C. Confirm the active context and obtain only the declared role boundary: Network Contributor and Storage Account Contributor on the lab resource group
- D. Skip authorization checks because all resources use an AZ104 naming prefix.

## 4. The setup command for Lab 19 is run without its execution switch. What should happen? (`LAB19-Q04`)
...
## 5. Which validation evidence most directly proves the baseline resource boundary for Lab 19? (`LAB19-Q05`)

- A. Query the exact recorded scope and verify the intended resource/object types, including Microsoft.Network/privateEndpoints, Microsoft.Network/privateDnsZones.
...
## 6. A learner reaches the externally gated checkpoint in Lab 19. What is the safest response? (`LAB19-Q06`)
...
## 7. Which command-side evidence best supports the first completed checkpoint in Lab 19? (`LAB19-Q07`)
...
- C. Run the independent validator and retain redacted structured output for the exact recorded scope after: Create separate service-endpoint and private-endpoint subnets with appropriate policies.
- D. Use the setup process exit code alone without querying resource state.

## 8. During cleanup of Lab 19, several similarly named resources exist. Which targeting method is correct? (`LAB19-Q08`)
...
## 9. Setup completed for Lab 19, but one required state check fails. What is the best break/fix approach? (`LAB19-Q09`)
...
## 10. Which completion claim is valid immediately after this repository's offline tests pass for Lab 19? (`LAB19-Q10`)
