...
Write-Host 'Lab: LAB-05'
Write-Host "Tenant: $($account.tenantId)"
Write-Host "Subscription: $SubscriptionId"
Write-Host "Location: $Location"
Write-Host 'Cost class: none'
Write-Host 'Role boundary: Resource Policy Contributor at the lab scope; Cost Management Contributor for the optional budget path'
...
$ResourceGroupName = "rg-az104-l05-$RunId"
$suffix = (($RunId -replace '[^a-z0-9]', '') + '000000000000').Substring(0, 12)

Write-Host 'LAB-05 plan'
Write-Host "  subscription: $SubscriptionId"
Write-Host "  location: $Location"
Write-Host '  scope: subscription'
Write-Host '  cost class: none'
Write-Host '  resources: resource group, policy assignment, optional cost budget'
if (-not $Execute) {
...
if ('subscription' -eq 'local') {
    ...
    if ($resourceGroup.tags.purpose -eq 'az104-lab' -and $resourceGroup.tags.labId -eq '05' -and $resourceGroup.tags.runId -eq $RunId) {
...
$output = [ordered]@{ labId = 'LAB-05'; runId = $RunId; generatedAt = (Get-Date).ToUniversalTime().ToString('o'); result = $result; checks = @($checks) }
...
if (-not $SubscriptionId) { $SubscriptionId = [string]$account.id }
if ($account.id -ne $SubscriptionId -or $state.subscriptionId -ne $SubscriptionId) { throw 'Active or recorded subscription mismatch.' }
Write-Host 'Cleanup preview for LAB-05'
...
if ($resourceGroup.id -ne $state.resourceGroup.id -or $resourceGroup.tags.purpose -ne 'az104-lab' -or $resourceGroup.tags.labId -ne '05' -or $resourceGroup.tags.runId -ne $RunId) {
...
```ps1
pwsh ./scripts/cli/Setup.ps1 -RunId az104l05-01 -SubscriptionId <subscription-id> -Location <region>
```
...
```ps1
pwsh ./scripts/cli/Setup.ps1 -RunId az104l05-01 -SubscriptionId <subscription-id> -Location <region> -Execute
```
...
```ps1
pwsh ./scripts/cli/Validate.ps1 -RunId az104l05-01 -SubscriptionId <subscription-id>
```
...
```ps1
pwsh ./scripts/cli/Cleanup.ps1 -RunId az104l05-01 -SubscriptionId <subscription-id>
```

After verifying every printed target belongs to this run:

```ps1
pwsh ./scripts/cli/Cleanup.ps1 -RunId az104l05-01 -SubscriptionId <subscription-id> -Execute
```
