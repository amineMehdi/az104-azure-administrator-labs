...
## 1. Which statement best captures the key design principle for enforce policy and manage costs with budgets and advisor? (`LAB05-Q01`)
...
## 2. You must begin the hands-on path for 'Implement and manage Azure Policy'. Which action matches the reviewed lab sequence? (`LAB05-Q02`)
...
## 3. Which authorization statement is appropriate before creating resource group, policy assignment, optional cost budget in this lab? (`LAB05-Q03`)
...
- C. Confirm the active context and obtain only the declared role boundary: Resource Policy Contributor at the lab scope; Cost Management Contributor for the optional budget path
- D. Skip authorization checks because all resources use an AZ104 naming prefix.

## 4. The setup command for Lab 05 is run without its execution switch. What should happen? (`LAB05-Q04`)
...
## 5. Which validation evidence most directly proves the baseline resource boundary for Lab 05? (`LAB05-Q05`)

- A. Query the exact recorded scope and verify the intended resource/object types, including Microsoft.Authorization/policyAssignments.
...
## 6. A learner reaches the externally gated checkpoint in Lab 05. What is the safest response? (`LAB05-Q06`)

- A. Invent a placeholder tenant, email address, domain, or production target and continue.
- B. Stop that branch unless its prerequisites are explicitly satisfied: Budget creation requires AZ104_BUDGET_EMAIL and Cost Management authorization; otherwise the lab inventories cost controls without creating one.
...
## 7. Which command-side evidence best supports the first completed checkpoint in Lab 05? (`LAB05-Q07`)
...
- C. Run the independent validator and retain redacted structured output for the exact recorded scope after: Create the tagged policy-test resource group.
- D. Use the setup process exit code alone without querying resource state.

## 8. During cleanup of Lab 05, several similarly named resources exist. Which targeting method is correct? (`LAB05-Q08`)
...
## 9. Setup completed for Lab 05, but one required state check fails. What is the best break/fix approach? (`LAB05-Q09`)
...
## 10. Which completion claim is valid immediately after this repository's offline tests pass for Lab 05? (`LAB05-Q10`)
