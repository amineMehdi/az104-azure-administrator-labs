...
[Previous: Lab 12](../12-vm-resilience-scale-mobility/README.md) · [Catalog](../README.md) · [Next: Lab 14](../14-container-apps/README.md)
...
Scenario: You are a container administrator publishing an approved image and running it in aci. Build and prove this disposable service path: An ACR build produces the run-tagged image, a user-assigned identity receives AcrPull at registry scope, and ACI uses that identity to pull and run the image without registry passwords.

Learner role: A container administrator publishing an approved image and running it in ACI.

Outcome: Use Azure CLI in PowerShell to create the private-admin acr registry, publish and identify the run-owned container image, and reconcile image provenance, identity, and container state, with recoverable state and deterministic cleanup.
...
|---|---|---|
| `CP-CONTAINERS-01` | Create and manage an Azure Container Registry | `LAB13-CP01`, `LAB13-CP04`, `LAB13-CP05` |
| `CP-CONTAINERS-02` | Provision a container by using Azure Container Instances | `LAB13-CP02`, `LAB13-CP05` |
| `CP-CONTAINERS-04` | Manage sizing and scaling for containers, including Azure Container Instances and Azure Container Apps | `LAB13-CP03`, `LAB13-CP05` |
...
![Lab 13 service topology](diagrams/architecture.svg)

The editable source is [architecture.mmd](diagrams/architecture.mmd). An ACR build produces the run-tagged image, a user-assigned identity receives AcrPull at registry scope, and ACI uses that identity to pull and run the image without registry passwords.

## Concept primer and design decisions

ACR is the managed image registry; ACI runs container groups without an orchestrator. The image reference, registry authentication, CPU, memory, ports, restart policy, and logs form the operational contract.
...
|---|:---:|---|---|---|
| `run-id` — Unique lowercase run ownership identifier | Yes | parameter | `az104l13-01` | `block` |
...
./scripts/cli/Preflight.ps1 -SubscriptionId $env:AZ104_SUBSCRIPTION_ID -RunId 'az104l13-01' -Location 'westeurope'
```
...
## Task 1 — Verify ACR and ACI regional capability {#task-1}

Checkpoint: `LAB13-CP01`

Purpose and operational relevance: Read the provider, region, SKU, feature, quota, and companion-tool signals needed for verify acr and aci regional capability, without changing Azure state. The change is unsafe when aCI image pulls can fail in a region even when ACR itself is available there. This checkpoint therefore proves: The selected region accepts registry and container-group discovery, and required providers are registered.
...
Expected state: The selected region accepts registry and container-group discovery, and required providers are registered.

Representative redacted output:

```text
The selected region accepts registry and container-group discovery, and required providers are registered.
```

Positive validation:

```powershell
$registration = az provider show --namespace Microsoft.ContainerRegistry --query registrationState --output tsv
if ($registration -ne 'Registered') { throw 'Microsoft.ContainerRegistry is not registered.' }
```

Expected positive result: The selected region accepts registry and container-group discovery, and required providers are registered.
...
Expected negative result: LAB13-CP01 negative boundary: The undesired state is absent; the negative assertion must not report: Microsoft.ContainerInstance is not registered.
...
- Positive assertion proving: The selected region accepts registry and container-group discovery, and required providers are registered.
- Negative assertion proving absence of: Microsoft.ContainerInstance is not registered.

Common failure and safe retry: ACI image pulls can fail in a region even when ACR itself is available there. Retry LAB13-CP01 for verify acr and aci regional capability only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After LAB13-CP02, LAB13-CP03, LAB13-CP04, LAB13-CP05, remove the exact manifest IDs created or configured by LAB13-CP01 for verify acr and aci regional capability; its residual probe must then return zero active IDs.

## Task 2 — Create the private-admin ACR registry {#task-2}

Checkpoint: `LAB13-CP02`

Purpose and operational relevance: Provision and immediately inventory the exact run-owned resources needed to create the private-admin acr registry, so partial completion remains recoverable. The change is unsafe when a globally unique registry-name collision or unregistered ContainerRegistry provider blocks creation. This checkpoint therefore proves: The Basic registry is ready, admin credentials remain disabled, and its exact login server is recorded.
...
Expected state: The Basic registry is ready, admin credentials remain disabled, and its exact login server is recorded.

Representative redacted output:

```text
The Basic registry is ready, admin credentials remain disabled, and its exact login server is recorded.
```

Positive validation:

```powershell
$tag = az acr repository show-tags --name $registry --repository az104/hello --query "[?@=='v1'] | [0]" --output tsv
if ($tag -ne 'v1') { throw 'The v1 image tag is missing.' }
```

Expected positive result: The Basic registry is ready, admin credentials remain disabled, and its exact login server is recorded.

Negative validation:

```powershell
$admin = az acr show --resource-group $ResourceGroupName --name $registry --query adminUserEnabled --output tsv
if ($admin -ne 'false') { throw 'The ACR admin account is enabled.' }
```

Expected negative result: LAB13-CP02 negative boundary: The undesired state is absent; the negative assertion must not report: The ACR admin account is enabled.
...
- Positive assertion proving: The Basic registry is ready, admin credentials remain disabled, and its exact login server is recorded.
- Negative assertion proving absence of: The ACR admin account is enabled.

Common failure and safe retry: A globally unique registry-name collision or unregistered ContainerRegistry provider blocks creation. Retry LAB13-CP02 for create the private-admin acr registry only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After LAB13-CP03, LAB13-CP04, LAB13-CP05, remove the exact manifest IDs created or configured by LAB13-CP02 for create the private-admin acr registry; its residual probe must then return zero active IDs.

## Task 3 — Publish and identify the run-owned container image {#task-3}

Checkpoint: `LAB13-CP03`

Purpose and operational relevance: Configure and independently inspect the control-plane and data-path properties needed to publish and identify the run-owned container image. The change is unsafe when a build can fail when the registry cannot reach the build context or the tag differs from the deployment reference. This checkpoint therefore proves: The tagged image manifest exists in ACR and is addressable by the immutable run-specific repository tag.
...
Expected state: The tagged image manifest exists in ACR and is addressable by the immutable run-specific repository tag.

Representative redacted output:

```text
The tagged image manifest exists in ACR and is addressable by the immutable run-specific repository tag.
```

Positive validation:

```powershell
$image = az container show --resource-group $ResourceGroupName --name $container --query "containers[0].image" --output tsv
if ($image -ne "$registry.azurecr.io/az104/hello:v1") { throw "Unexpected image: $image" }
```

Expected positive result: The tagged image manifest exists in ACR and is addressable by the immutable run-specific repository tag.
...
Expected negative result: LAB13-CP03 negative boundary: The undesired state is absent; the negative assertion must not report: The ACR admin account is enabled.
...
- Positive assertion proving: The tagged image manifest exists in ACR and is addressable by the immutable run-specific repository tag.
- Negative assertion proving absence of: The ACR admin account is enabled.

Common failure and safe retry: A build can fail when the registry cannot reach the build context or the tag differs from the deployment reference. Retry LAB13-CP03 for publish and identify the run-owned container image only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After LAB13-CP04, LAB13-CP05, remove the exact manifest IDs created or configured by LAB13-CP03 for publish and identify the run-owned container image; its residual probe must then return zero active IDs.

## Task 4 — Run ACI with managed identity image access {#task-4}

Checkpoint: `LAB13-CP04`

Purpose and operational relevance: Exercise the deterministic operational or denied path needed to run aci with managed identity image access, then retain comparable before-and-after results. The change is unsafe when identity propagation delay can cause an initial ACR pull denial even after the role assignment exists. This checkpoint therefore proves: The container group reaches Succeeded with a managed identity and an AcrPull assignment scoped to the registry.
...
Expected state: The container group reaches Succeeded with a managed identity and an AcrPull assignment scoped to the registry.

Representative redacted output:

```text
The container group reaches Succeeded with a managed identity and an AcrPull assignment scoped to the registry.
```

Positive validation:

```powershell
$stateValue = az container show --resource-group $ResourceGroupName --name $container --query "containers[0].instanceView.currentState.state" --output tsv
if ($stateValue -notin @('Running','Terminated')) { throw "Container state is $stateValue." }
```

Expected positive result: The container group reaches Succeeded with a managed identity and an AcrPull assignment scoped to the registry.

Negative validation:

```powershell
$previous = az container show --resource-group $ResourceGroupName --name $container --query "containers[0].instanceView.previousState.state" --output tsv
if ($previous -eq 'Failed') { throw 'The previous container state failed.' }
```

Expected negative result: LAB13-CP04 negative boundary: The undesired state is absent; the negative assertion must not report: The previous container state failed.
...
- Positive assertion proving: The container group reaches Succeeded with a managed identity and an AcrPull assignment scoped to the registry.
- Negative assertion proving absence of: The previous container state failed.

Common failure and safe retry: Identity propagation delay can cause an initial ACR pull denial even after the role assignment exists. Retry LAB13-CP04 for run aci with managed identity image access only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After LAB13-CP05, remove the exact manifest IDs created or configured by LAB13-CP04 for run aci with managed identity image access; its residual probe must then return zero active IDs.

## Task 5 — Reconcile image provenance, identity, and container state {#task-5}

Checkpoint: `LAB13-CP05`

Purpose and operational relevance: Reconcile live service properties, returned IDs, and dependency-aware removal needed to reconcile image provenance, identity, and container state before handoff. The change is unsafe when using registry passwords would bypass the intended identity path and leave secret-bearing evidence. This checkpoint therefore proves: The ACI image reference points to the run-owned registry, admin is disabled, and the container instance reports Running or Succeeded.
...
Expected state: The ACI image reference points to the run-owned registry, admin is disabled, and the container instance reports Running or Succeeded.

Representative redacted output:

```text
The ACI image reference points to the run-owned registry, admin is disabled, and the container instance reports Running or Succeeded.
```

Positive validation:

```powershell
$provisioning = az container show --resource-group $ResourceGroupName --name $container --query provisioningState --output tsv
if ($provisioning -ne 'Succeeded') { throw "ACI provisioning state is $provisioning." }
```

Expected positive result: The ACI image reference points to the run-owned registry, admin is disabled, and the container instance reports Running or Succeeded.

Negative validation:

```powershell
$restartCount = az container show --resource-group $ResourceGroupName --name $container --query "containers[0].instanceView.restartCount" --output tsv
if ($restartCount -and [int]$restartCount -gt 3) { throw "Container restarted $restartCount times." }
```

Expected negative result: LAB13-CP05 negative boundary: The undesired state is absent; the negative assertion must not report: Container restarted reported restartCount times.
...
- Positive assertion proving: The ACI image reference points to the run-owned registry, admin is disabled, and the container instance reports Running or Succeeded.
- Negative assertion proving absence of: Container restarted reported restartCount times.

Common failure and safe retry: Using registry passwords would bypass the intended identity path and leave secret-bearing evidence. Retry LAB13-CP05 for reconcile image provenance, identity, and container state only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After all service validation and evidence capture, remove the exact manifest IDs created or configured by LAB13-CP05 for reconcile image provenance, identity, and container state; its residual probe must then return zero active IDs.
...
```powershell
./scripts/cli/Validate.ps1 -RunId 'az104l13-01' -Mode Deployment
az resource list --resource-group 'rg-az104-l13-az104l13-01' --query '[].{name:name,type:type}' --output table
```
...
az container create --resource-group $ResourceGroupName --name "aci-fault-$suffix" --image "$registry.azurecr.io/az104/hello:missing" --cpu 1 --memory 1 --restart-policy Never --output none
...
```powershell
az container show --resource-group $ResourceGroupName --name "aci-fault-$suffix" --query "containers[0].instanceView.events[].{name:name,message:message}" --output table
```

Repair only the injected setting, then repeat the same query:

```powershell
az container delete --resource-group $ResourceGroupName --name "aci-fault-$suffix" --yes --output none
az container show --resource-group $ResourceGroupName --name $container --query "{image:containers[0].image,state:containers[0].instanceView.currentState.state}" --output json
```

Expected symptom: The container group remains pending or terminates with an image-pull error.
...
Promote v1 to v2 while keeping an explicit rollback tag and compare ACI restart behavior.
...
```powershell
./scripts/cli/Cleanup.ps1 -RunId 'az104l13-01'
./scripts/cli/Cleanup.ps1 -RunId 'az104l13-01' -Execute
./scripts/cli/Validate.ps1 -RunId 'az104l13-01' -Mode PostCleanup
$restartCount = az container show --resource-group $ResourceGroupName --name $container --query "containers[0].instanceView.restartCount" --output tsv
if ($restartCount -and [int]$restartCount -gt 3) { throw "Container restarted $restartCount times." }
```
...
- [Create an Azure container registry with Azure CLI](https://learn.microsoft.com/en-us/azure/container-registry/container-registry-get-started-azure-cli)
- [Import container images into Azure Container Registry](https://learn.microsoft.com/en-us/azure/container-registry/container-registry-import-images)
- [Authenticate ACI to ACR with managed identity](https://learn.microsoft.com/en-us/azure/container-instances/using-azure-container-registry-mi)
...
if ($state.labId -ne 'LAB-13' -or $state.runId -ne $RunId) { throw 'Manifest ownership does not match this lab and run ID.' }
$PSNativeCommandUseErrorActionPreference = $false
...
# CHECKPOINT LAB13-CP01 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB13-CP01' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP01' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab13-cp01.positive' -CheckpointId 'LAB13-CP01' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab13-cp01.negative' -CheckpointId 'LAB13-CP01' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
...
if ($registration -ne 'Registered') { throw 'Microsoft.ContainerRegistry is not registered.' }
    ...
    Add-ValidationCheck -Id 'lab13-cp01.positive' -CheckpointId 'LAB13-CP01' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $positivePassed -Command 'query each manifest-recorded object by exact ID and run the authored service probe' -Expected 'Checkpoint state is pass, recorded objects exist, and service state matches.' -Actual $positiveActual
...
if ($registration -ne 'Registered') { throw 'Microsoft.ContainerInstance is not registered.' }
    ...
    Add-ValidationCheck -Id 'lab13-cp01.negative' -CheckpointId 'LAB13-CP01' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    ...
    Add-ValidationCheck -Id 'lab13-cp01.residual' -CheckpointId 'LAB13-CP01' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB13-CP01 END

# CHECKPOINT LAB13-CP02 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB13-CP02' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP02' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab13-cp02.positive' -CheckpointId 'LAB13-CP02' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab13-cp02.negative' -CheckpointId 'LAB13-CP02' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
    $positivePassed = $checkpointState.status -eq 'pass'
    $positiveActual = "checkpoint=$($checkpointState.status); managedObjects=$($checkpointObjects.Count)"
    foreach ($managedObject in $checkpointObjects) {
...
if ($tag -ne 'v1') { throw 'The v1 image tag is missing.' }
    ...
    Add-ValidationCheck -Id 'lab13-cp02.positive' -CheckpointId 'LAB13-CP02' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $positivePassed -Command 'query each manifest-recorded object by exact ID and run the authored service probe' -Expected 'Checkpoint state is pass, recorded objects exist, and service state matches.' -Actual $positiveActual
...
if ($admin -ne 'false') { throw 'The ACR admin account is enabled.' }
    ...
    $negativePassed = $unsafe.Count -eq 0 -and $negativeProbePassed
    $negativeActual = "unsafeObjects=$($unsafe.Count)" + "; $negativeProbeActual"
    Add-ValidationCheck -Id 'lab13-cp02.negative' -CheckpointId 'LAB13-CP02' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    $active = [System.Collections.Generic.List[string]]::new()
    foreach ($managedObject in $checkpointObjects) {
    ...
    Add-ValidationCheck -Id 'lab13-cp02.residual' -CheckpointId 'LAB13-CP02' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB13-CP02 END

# CHECKPOINT LAB13-CP03 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB13-CP03' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP03' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab13-cp03.positive' -CheckpointId 'LAB13-CP03' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab13-cp03.negative' -CheckpointId 'LAB13-CP03' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
    $positivePassed = $checkpointState.status -eq 'pass'
    $positiveActual = "checkpoint=$($checkpointState.status); managedObjects=$($checkpointObjects.Count)"
    foreach ($managedObject in $checkpointObjects) {
...
if ($image -ne "$registry.azurecr.io/az104/hello:v1") { throw "Unexpected image: $image" }
    ...
    Add-ValidationCheck -Id 'lab13-cp03.positive' -CheckpointId 'LAB13-CP03' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $positivePassed -Command 'query each manifest-recorded object by exact ID and run the authored service probe' -Expected 'Checkpoint state is pass, recorded objects exist, and service state matches.' -Actual $positiveActual
...
if ($admin -ne 'false') { throw 'The ACR admin account is enabled.' }
    ...
    $negativePassed = $unsafe.Count -eq 0 -and $negativeProbePassed
    $negativeActual = "unsafeObjects=$($unsafe.Count)" + "; $negativeProbeActual"
    Add-ValidationCheck -Id 'lab13-cp03.negative' -CheckpointId 'LAB13-CP03' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    $active = [System.Collections.Generic.List[string]]::new()
    foreach ($managedObject in $checkpointObjects) {
    ...
    Add-ValidationCheck -Id 'lab13-cp03.residual' -CheckpointId 'LAB13-CP03' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB13-CP03 END

# CHECKPOINT LAB13-CP04 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB13-CP04' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP04' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab13-cp04.positive' -CheckpointId 'LAB13-CP04' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab13-cp04.negative' -CheckpointId 'LAB13-CP04' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
    $positivePassed = $checkpointState.status -eq 'pass'
    $positiveActual = "checkpoint=$($checkpointState.status); managedObjects=$($checkpointObjects.Count)"
    foreach ($managedObject in $checkpointObjects) {
...
if ($stateValue -notin @('Running','Terminated')) { throw "Container state is $stateValue." }
    ...
    Add-ValidationCheck -Id 'lab13-cp04.positive' -CheckpointId 'LAB13-CP04' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $positivePassed -Command 'query each manifest-recorded object by exact ID and run the authored service probe' -Expected 'Checkpoint state is pass, recorded objects exist, and service state matches.' -Actual $positiveActual
...
if ($previous -eq 'Failed') { throw 'The previous container state failed.' }
    ...
    $negativePassed = $unsafe.Count -eq 0 -and $negativeProbePassed
    $negativeActual = "unsafeObjects=$($unsafe.Count)" + "; $negativeProbeActual"
    Add-ValidationCheck -Id 'lab13-cp04.negative' -CheckpointId 'LAB13-CP04' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    $active = [System.Collections.Generic.List[string]]::new()
    foreach ($managedObject in $checkpointObjects) {
    ...
    Add-ValidationCheck -Id 'lab13-cp04.residual' -CheckpointId 'LAB13-CP04' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB13-CP04 END

# CHECKPOINT LAB13-CP05 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB13-CP05' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP05' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab13-cp05.positive' -CheckpointId 'LAB13-CP05' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab13-cp05.negative' -CheckpointId 'LAB13-CP05' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
    $positivePassed = $checkpointState.status -eq 'pass'
    $positiveActual = "checkpoint=$($checkpointState.status); managedObjects=$($checkpointObjects.Count)"
    foreach ($managedObject in $checkpointObjects) {
...
if ($restartCount -and [int]$restartCount -gt 3) { throw "Container restarted $restartCount times." }
    ...
    $negativePassed = $unsafe.Count -eq 0 -and $negativeProbePassed
    $negativeActual = "unsafeObjects=$($unsafe.Count)" + "; $negativeProbeActual"
    Add-ValidationCheck -Id 'lab13-cp05.negative' -CheckpointId 'LAB13-CP05' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    $active = [System.Collections.Generic.List[string]]::new()
    foreach ($managedObject in $checkpointObjects) {
    ...
    Add-ValidationCheck -Id 'lab13-cp05.residual' -CheckpointId 'LAB13-CP05' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB13-CP05 END
...
function Write-CleanupRefusal {
    ...
    $refusal = @{
        schemaVersion = '1.0.0'; labId = 'LAB-13'; runId = $RunId
        generatedAt = (Get-Date).ToUniversalTime().ToString('o')
...
if ($state.labId -ne 'LAB-13' -or $state.runId -ne $RunId) {
...
foreach ($object in $active) {
    ...
    if ($method -eq 'manifest-id-and-tags' -and $object.kind -eq 'azure-resource') {
        ...
        $manifestTagsMatch =
            ...
            [string]$expectedTags['labId'] -eq '13' -and
...
# CHECKPOINT LAB13-CP05 BEGIN
$targets = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP05' -and $_.lifecycleStatus -eq 'active' })
foreach ($target in $targets) {
    $alreadyAbsent = $absentIds.Contains([string]$target.id)
    $actions.Add(@{ checkpointId = 'LAB13-CP05'; targetId = [string]$target.id; targetType = [string]$target.type; ownership = @{ method = [string]$target.ownership.method; verified = [bool]$verifiedOwnershipById[[string]$target.id] }; status = $(if ($alreadyAbsent) { 'skipped' } else { 'preview' }); message = $(if ($alreadyAbsent) { 'The exact manifest ID was already absent; idempotent cleanup recorded the safe end state.' } elseif ($Execute) { 'Pending removal through the dependency-safe ownership boundary.' } else { 'Would remove this exact manifest-recorded target.' }) })
}
$state.updatedAt = (Get-Date).ToUniversalTime().ToString('o')
$state | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Manifest -Encoding utf8
# CHECKPOINT LAB13-CP05 END
...
# CHECKPOINT LAB13-CP04 BEGIN
$targets = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP04' -and $_.lifecycleStatus -eq 'active' })
foreach ($target in $targets) {
    $alreadyAbsent = $absentIds.Contains([string]$target.id)
    $actions.Add(@{ checkpointId = 'LAB13-CP04'; targetId = [string]$target.id; targetType = [string]$target.type; ownership = @{ method = [string]$target.ownership.method; verified = [bool]$verifiedOwnershipById[[string]$target.id] }; status = $(if ($alreadyAbsent) { 'skipped' } else { 'preview' }); message = $(if ($alreadyAbsent) { 'The exact manifest ID was already absent; idempotent cleanup recorded the safe end state.' } elseif ($Execute) { 'Pending removal through the dependency-safe ownership boundary.' } else { 'Would remove this exact manifest-recorded target.' }) })
}
$state.updatedAt = (Get-Date).ToUniversalTime().ToString('o')
$state | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Manifest -Encoding utf8
# CHECKPOINT LAB13-CP04 END
...
# CHECKPOINT LAB13-CP03 BEGIN
$targets = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP03' -and $_.lifecycleStatus -eq 'active' })
foreach ($target in $targets) {
    $alreadyAbsent = $absentIds.Contains([string]$target.id)
    $actions.Add(@{ checkpointId = 'LAB13-CP03'; targetId = [string]$target.id; targetType = [string]$target.type; ownership = @{ method = [string]$target.ownership.method; verified = [bool]$verifiedOwnershipById[[string]$target.id] }; status = $(if ($alreadyAbsent) { 'skipped' } else { 'preview' }); message = $(if ($alreadyAbsent) { 'The exact manifest ID was already absent; idempotent cleanup recorded the safe end state.' } elseif ($Execute) { 'Pending removal through the dependency-safe ownership boundary.' } else { 'Would remove this exact manifest-recorded target.' }) })
}
$state.updatedAt = (Get-Date).ToUniversalTime().ToString('o')
$state | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Manifest -Encoding utf8
# CHECKPOINT LAB13-CP03 END
...
# CHECKPOINT LAB13-CP02 BEGIN
$targets = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP02' -and $_.lifecycleStatus -eq 'active' })
foreach ($target in $targets) {
    $alreadyAbsent = $absentIds.Contains([string]$target.id)
    $actions.Add(@{ checkpointId = 'LAB13-CP02'; targetId = [string]$target.id; targetType = [string]$target.type; ownership = @{ method = [string]$target.ownership.method; verified = [bool]$verifiedOwnershipById[[string]$target.id] }; status = $(if ($alreadyAbsent) { 'skipped' } else { 'preview' }); message = $(if ($alreadyAbsent) { 'The exact manifest ID was already absent; idempotent cleanup recorded the safe end state.' } elseif ($Execute) { 'Pending removal through the dependency-safe ownership boundary.' } else { 'Would remove this exact manifest-recorded target.' }) })
}
$state.updatedAt = (Get-Date).ToUniversalTime().ToString('o')
$state | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Manifest -Encoding utf8
# CHECKPOINT LAB13-CP02 END
...
# CHECKPOINT LAB13-CP01 BEGIN
$targets = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP01' -and $_.lifecycleStatus -eq 'active' })
foreach ($target in $targets) {
    $alreadyAbsent = $absentIds.Contains([string]$target.id)
    $actions.Add(@{ checkpointId = 'LAB13-CP01'; targetId = [string]$target.id; targetType = [string]$target.type; ownership = @{ method = [string]$target.ownership.method; verified = [bool]$verifiedOwnershipById[[string]$target.id] }; status = $(if ($alreadyAbsent) { 'skipped' } else { 'preview' }); message = $(if ($alreadyAbsent) { 'The exact manifest ID was already absent; idempotent cleanup recorded the safe end state.' } elseif ($Execute) { 'Pending removal through the dependency-safe ownership boundary.' } else { 'Would remove this exact manifest-recorded target.' }) })
}
$state.updatedAt = (Get-Date).ToUniversalTime().ToString('o')
$state | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Manifest -Encoding utf8
# CHECKPOINT LAB13-CP01 END
...
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP01' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp01.residual'; command = 'query manifest-recorded IDs for LAB13-CP01'; expected = 'zero active objects for LAB13-CP01'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP02' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp02.residual'; command = 'query manifest-recorded IDs for LAB13-CP02'; expected = 'zero active objects for LAB13-CP02'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP03' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp03.residual'; command = 'query manifest-recorded IDs for LAB13-CP03'; expected = 'zero active objects for LAB13-CP03'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP04' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp04.residual'; command = 'query manifest-recorded IDs for LAB13-CP04'; expected = 'zero active objects for LAB13-CP04'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB13-CP05' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp05.residual'; command = 'query manifest-recorded IDs for LAB13-CP05'; expected = 'zero active objects for LAB13-CP05'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
