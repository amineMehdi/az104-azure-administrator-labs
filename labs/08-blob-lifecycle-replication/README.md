...
|---|---|
| `ST-ACCOUNTS-03` | Configure object replication |
| `ST-ACCOUNTS-05` | Manage data by using Azure Storage Explorer and AzCopy |
| `ST-DATA-02` | Create and configure a container in Azure Blob Storage |
| `ST-DATA-03` | Configure storage tiers |
| `ST-DATA-04` | Configure soft delete for blobs and containers |
| `ST-DATA-06` | Configure blob lifecycle management |
| `ST-DATA-07` | Configure blob versioning |
...
![Lab 08 architecture](diagrams/architecture.svg)
...
The key design idea is: **Object replication depends on versioning and change feed, while lifecycle rules act asynchronously and should be validated as configuration rather than immediate data movement.**
...
| Required boundary | Contributor and Storage Blob Data Contributor on the lab resource group |
...
|---:|---|
| 1 | resource group |
| 2 | two StorageV2 accounts |
| 3 | blob containers |
| 4 | lifecycle policy |
| 5 | object replication policy |
...
- Choose a unique run ID matching `^[a-z0-9-]+$`, such as `az104l08-01`.
...
Write-Host 'Lab: LAB-08'
Write-Host "Tenant: $($account.tenantId)"
Write-Host "Subscription: $SubscriptionId"
Write-Host "Location: $Location"
Write-Host 'Cost class: low'
Write-Host 'Role boundary: Contributor and Storage Blob Data Contributor on the lab resource group'
...
$ResourceGroupName = "rg-az104-l08-$RunId"
$suffix = (($RunId -replace '[^a-z0-9]', '') + '000000000000').Substring(0, 12)

Write-Host 'LAB-08 plan'
Write-Host "  subscription: $SubscriptionId"
Write-Host "  location: $Location"
Write-Host '  scope: subscription'
Write-Host '  cost class: low'
Write-Host '  resources: resource group, two StorageV2 accounts, blob containers, lifecycle policy, object replication policy'
if (-not $Execute) {
...
if ('subscription' -eq 'local') {
    ...
    if ($resourceGroup.tags.purpose -eq 'az104-lab' -and $resourceGroup.tags.labId -eq '08' -and $resourceGroup.tags.runId -eq $RunId) {
...
$output = [ordered]@{ labId = 'LAB-08'; runId = $RunId; generatedAt = (Get-Date).ToUniversalTime().ToString('o'); result = $result; checks = @($checks) }
...
if (-not $SubscriptionId) { $SubscriptionId = [string]$account.id }
if ($account.id -ne $SubscriptionId -or $state.subscriptionId -ne $SubscriptionId) { throw 'Active or recorded subscription mismatch.' }
Write-Host 'Cleanup preview for LAB-08'
...
if ($resourceGroup.id -ne $state.resourceGroup.id -or $resourceGroup.tags.purpose -ne 'az104-lab' -or $resourceGroup.tags.labId -ne '08' -or $resourceGroup.tags.runId -ne $RunId) {
...
```powershell
pwsh ./scripts/cli/Setup.ps1 -RunId az104l08-01 -SubscriptionId <subscription-id> -Location <region>
```
...
```powershell
pwsh ./scripts/cli/Setup.ps1 -RunId az104l08-01 -SubscriptionId <subscription-id> -Location <region> -Execute
```
...
```powershell
pwsh ./scripts/cli/Validate.ps1 -RunId az104l08-01 -SubscriptionId <subscription-id>
```
...
```powershell
pwsh ./scripts/cli/Cleanup.ps1 -RunId az104l08-01 -SubscriptionId <subscription-id>
```

After verifying every printed target belongs to this run:

```powershell
pwsh ./scripts/cli/Cleanup.ps1 -RunId az104l08-01 -SubscriptionId <subscription-id> -Execute
```
...
- [https://learn.microsoft.com/en-us/azure/storage/blobs/versioning-overview](https://learn.microsoft.com/en-us/azure/storage/blobs/versioning-overview)
- [https://learn.microsoft.com/en-us/azure/storage/blobs/soft-delete-blob-overview](https://learn.microsoft.com/en-us/azure/storage/blobs/soft-delete-blob-overview)
...
- [https://learn.microsoft.com/en-us/azure/storage/common/storage-use-azcopy-v10](https://learn.microsoft.com/en-us/azure/storage/common/storage-use-azcopy-v10)
