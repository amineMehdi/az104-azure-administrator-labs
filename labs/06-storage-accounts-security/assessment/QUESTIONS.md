...
## 1. Which statement best captures the key design principle for secure storage accounts, redundancy, encryption, and keys? (`LAB06-Q01`)
...
## 2. You must begin the hands-on path for 'Manage access keys'. Which action matches the reviewed lab sequence? (`LAB06-Q02`)

- A. Delete similarly named resources before recording the current subscription.
- B. Create a globally unique StorageV2 account with Standard_ZRS when available and a documented LRS fallback.
...
## 3. Which authorization statement is appropriate before creating resource group, StorageV2 account in this lab? (`LAB06-Q03`)
...
## 4. The setup command for Lab 06 is run without its execution switch. What should happen? (`LAB06-Q04`)
...
## 5. Which validation evidence most directly proves the baseline resource boundary for Lab 06? (`LAB06-Q05`)
...
## 6. A learner reaches the externally gated checkpoint in Lab 06. What is the safest response? (`LAB06-Q06`)
...
## 7. Which command-side evidence best supports the first completed checkpoint in Lab 06? (`LAB06-Q07`)
...
- C. Run the independent validator and retain redacted structured output for the exact recorded scope after: Create a globally unique StorageV2 account with Standard_ZRS when available and a documented LRS fallback.
- D. Use the setup process exit code alone without querying resource state.

## 8. During cleanup of Lab 06, several similarly named resources exist. Which targeting method is correct? (`LAB06-Q08`)
...
## 9. Setup completed for Lab 06, but one required state check fails. What is the best break/fix approach? (`LAB06-Q09`)
...
## 10. Which completion claim is valid immediately after this repository's offline tests pass for Lab 06? (`LAB06-Q10`)
