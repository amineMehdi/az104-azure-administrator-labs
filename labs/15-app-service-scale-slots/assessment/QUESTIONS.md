...
## 1. Which statement best captures the key design principle for scale app service and operate deployment slots? (`LAB15-Q01`)

- A. App Service plan scale affects workers shared by apps, while deployment slots isolate deployable content and selected sticky configuration before a controlled swap.
...
## 2. You must begin the hands-on path for 'Provision an App Service plan'. Which action matches the reviewed lab sequence? (`LAB15-Q02`)

- A. Delete similarly named resources before recording the current subscription.
- B. Create a Linux App Service plan and web app with deterministic names.
...
## 3. Which authorization statement is appropriate before creating resource group, App Service plan, web app in this lab? (`LAB15-Q03`)
...
- C. Confirm the active context and obtain only the declared role boundary: Website Contributor and Monitoring Contributor on the lab resource group
- D. Skip authorization checks because all resources use an AZ104 naming prefix.

## 4. The setup command for Lab 15 is run without its execution switch. What should happen? (`LAB15-Q04`)
...
## 5. Which validation evidence most directly proves the baseline resource boundary for Lab 15? (`LAB15-Q05`)
...
## 6. A learner reaches the externally gated checkpoint in Lab 15. What is the safest response? (`LAB15-Q06`)
...
## 7. Which command-side evidence best supports the first completed checkpoint in Lab 15? (`LAB15-Q07`)
...
- C. Run the independent validator and retain redacted structured output for the exact recorded scope after: Create a Linux App Service plan and web app with deterministic names.
- D. Use the setup process exit code alone without querying resource state.

## 8. During cleanup of Lab 15, several similarly named resources exist. Which targeting method is correct? (`LAB15-Q08`)
...
## 9. Setup completed for Lab 15, but one required state check fails. What is the best break/fix approach? (`LAB15-Q09`)
...
## 10. Which completion claim is valid immediately after this repository's offline tests pass for Lab 15? (`LAB15-Q10`)
