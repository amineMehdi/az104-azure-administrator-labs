...
|---|---|
| `CP-APP-01` | Provision an App Service plan |
| `CP-APP-02` | Configure scaling for an App Service plan |
| `CP-APP-03` | Create an App Service |
| `CP-APP-08` | Configure deployment slots for an App Service |
...
![Lab 15 architecture](diagrams/architecture.svg)
...
The key design idea is: **App Service plan scale affects workers shared by apps, while deployment slots isolate deployable content and selected sticky configuration before a controlled swap.**
...
| Required boundary | Website Contributor and Monitoring Contributor on the lab resource group |
...
- Resource providers observed by preflight: `Microsoft.Insights`, `Microsoft.Web`
- PowerShell modules used when applicable: `Az.Accounts`, `Az.Resources`, `Az.Websites`, `Az.Monitor`
...
- Choose a unique run ID matching `^[a-z0-9-]+$`, such as `az104l15-01`.
...
Write-Host 'Lab: LAB-15'
Write-Host 'Location:' $Location
Write-Host 'Cost class: low'
Write-Host 'Role boundary: Website Contributor and Monitoring Contributor on the lab resource group'
$providers = @(
    'Microsoft.Insights',
    'Microsoft.Web'
)
...
$tags = @{ purpose = 'az104-lab'; labId = '15'; runId = $RunId; expiresOn = (Get-Date).ToUniversalTime().AddDays(1).ToString('yyyy-MM-dd') }
...
if ($rg -and $rg.Tags.purpose -eq 'az104-lab' -and $rg.Tags.labId -eq '15' -and $rg.Tags.runId -eq $RunId) {
...
$output = [ordered]@{ labId = 'LAB-15'; runId = $RunId; generatedAt = (Get-Date).ToUniversalTime().ToString('o'); result = $result; checks = @($checks) }
...
if (-not (Test-Path -LiteralPath $Manifest)) { throw "Missing state: $Manifest" }
$state = Get-Content -LiteralPath $Manifest -Raw | ConvertFrom-Json -Depth 20
Write-Host 'Cleanup preview for LAB-15'
...
if ($rg.ResourceId -ne $state.resourceGroup.id -or $rg.Tags.purpose -ne 'az104-lab' -or $rg.Tags.labId -ne '15' -or $rg.Tags.runId -ne $RunId) {
...
```ps1
pwsh ./scripts/powershell/Setup.ps1 -RunId az104l15-01 -SubscriptionId <subscription-id> -Location <region>
```
...
```ps1
pwsh ./scripts/powershell/Setup.ps1 -RunId az104l15-01 -SubscriptionId <subscription-id> -Location <region> -Execute
```
...
```ps1
pwsh ./scripts/powershell/Validate.ps1 -RunId az104l15-01 -SubscriptionId <subscription-id>
```
...
```ps1
pwsh ./scripts/powershell/Cleanup.ps1 -RunId az104l15-01 -SubscriptionId <subscription-id>
```

After verifying every printed target belongs to this run:

```ps1
pwsh ./scripts/powershell/Cleanup.ps1 -RunId az104l15-01 -SubscriptionId <subscription-id> -Execute
```
...
- [https://learn.microsoft.com/en-us/azure/app-service/quickstart-powershell](https://learn.microsoft.com/en-us/azure/app-service/quickstart-powershell)
- [https://learn.microsoft.com/en-us/azure/app-service/manage-scale-up](https://learn.microsoft.com/en-us/azure/app-service/manage-scale-up)
- [https://learn.microsoft.com/en-us/azure/app-service/manage-scale-per-app](https://learn.microsoft.com/en-us/azure/app-service/manage-scale-per-app)
- [https://learn.microsoft.com/en-us/azure/app-service/deploy-staging-slots](https://learn.microsoft.com/en-us/azure/app-service/deploy-staging-slots)
