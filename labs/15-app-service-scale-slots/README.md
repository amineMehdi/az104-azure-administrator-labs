...
# Lab 15: Scale App Service and operate deployment slots

[Previous: Lab 14](../14-container-apps/README.md) · [Catalog](../README.md) · [Next: Lab 16](../16-app-service-tls-dns-backup-network/README.md)
...
Scenario: You are an application-platform administrator operating app service capacity and releases. Build and prove this disposable service path: A Linux App Service plan hosts production and staging slots; plan autoscale changes worker capacity, sticky settings remain slot-scoped, and the staging release flows through a reversible swap into production.

Learner role: An application-platform administrator operating App Service capacity and releases.

Outcome: Use Azure CLI in PowerShell to create the production web app and deployment slot, configure scale-out and slot-sticky settings, and reconcile worker count, slot state, and configuration scope, with recoverable state and deterministic cleanup.
...
|---|---|---|
| `CP-APP-01` | Provision an App Service plan | `LAB15-CP01`, `LAB15-CP05` |
| `CP-APP-02` | Configure scaling for an App Service plan | `LAB15-CP02`, `LAB15-CP05` |
| `CP-APP-03` | Create an App Service | `LAB15-CP03`, `LAB15-CP05` |
| `CP-APP-08` | Configure deployment slots for an App Service | `LAB15-CP04`, `LAB15-CP05` |
...
![Lab 15 service topology](diagrams/architecture.svg)

The editable source is [architecture.mmd](diagrams/architecture.mmd). A Linux App Service plan hosts production and staging slots; plan autoscale changes worker capacity, sticky settings remain slot-scoped, and the staging release flows through a reversible swap into production.

## Concept primer and design decisions

App Service plan scale-up changes worker capability; scale-out changes instance count. Deployment slots isolate configuration and content for safer releases, while slot settings remain sticky during a swap.
...
|---|:---:|---|---|---|
| `run-id` — Unique lowercase run ownership identifier | Yes | parameter | `az104l15-01` | `block` |
...
./scripts/cli/Preflight.ps1 -SubscriptionId $env:AZ104_SUBSCRIPTION_ID -RunId 'az104l15-01' -Location 'westeurope'
```
...
## Task 1 — Verify App Service plan and slot capacity {#task-1}

Checkpoint: `LAB15-CP01`

Purpose and operational relevance: Read the provider, region, SKU, feature, quota, and companion-tool signals needed for verify app service plan and slot capacity, without changing Azure state. The change is unsafe when free or shared App Service tiers do not support the scaling and slot features used by this lab. This checkpoint therefore proves: The selected region supports the documented Linux plan SKU and the subscription can enumerate web-app deployment slots.
...
Expected state: The selected region supports the documented Linux plan SKU and the subscription can enumerate web-app deployment slots.

Representative redacted output:

```text
The selected region supports the documented Linux plan SKU and the subscription can enumerate web-app deployment slots.
```
...
Expected positive result: The selected region supports the documented Linux plan SKU and the subscription can enumerate web-app deployment slots.
...
Expected negative result: LAB15-CP01 negative boundary: The undesired state is absent; the negative assertion must not report: Linux S1 is unavailable in the selected region.
...
- Positive assertion proving: The selected region supports the documented Linux plan SKU and the subscription can enumerate web-app deployment slots.
- Negative assertion proving absence of: Linux S1 is unavailable in the selected region.

Common failure and safe retry: Free or shared App Service tiers do not support the scaling and slot features used by this lab. Retry LAB15-CP01 for verify app service plan and slot capacity only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After LAB15-CP02, LAB15-CP03, LAB15-CP04, LAB15-CP05, remove the exact manifest IDs created or configured by LAB15-CP01 for verify app service plan and slot capacity; its residual probe must then return zero active IDs.

## Task 2 — Create the production web app and deployment slot {#task-2}

Checkpoint: `LAB15-CP02`

Purpose and operational relevance: Provision and immediately inventory the exact run-owned resources needed to create the production web app and deployment slot, so partial completion remains recoverable. The change is unsafe when a globally unique app-name collision or incompatible runtime string blocks app creation. This checkpoint therefore proves: The production app and staging slot share the run-owned plan and each endpoint reports a healthy hostname.
...
Expected state: The production app and staging slot share the run-owned plan and each endpoint reports a healthy hostname.

Representative redacted output:

```text
The production app and staging slot share the run-owned plan and each endpoint reports a healthy hostname.
```

Positive validation:

```powershell
$slotCount = az webapp deployment slot list --resource-group $ResourceGroupName --name $app --query "[?name=='$app/staging'] | length(@)" --output tsv
if ([int]$slotCount -ne 1) { throw 'The staging slot does not exist exactly once.' }
```

Expected positive result: The production app and staging slot share the run-owned plan and each endpoint reports a healthy hostname.
...
Expected negative result: LAB15-CP02 negative boundary: The undesired state is absent; the negative assertion must not report: Web app state is reported stateValue.
...
- Positive assertion proving: The production app and staging slot share the run-owned plan and each endpoint reports a healthy hostname.
- Negative assertion proving absence of: Web app state is reported stateValue.

Common failure and safe retry: A globally unique app-name collision or incompatible runtime string blocks app creation. Retry LAB15-CP02 for create the production web app and deployment slot only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After LAB15-CP03, LAB15-CP04, LAB15-CP05, remove the exact manifest IDs created or configured by LAB15-CP02 for create the production web app and deployment slot; its residual probe must then return zero active IDs.

## Task 3 — Configure scale-out and slot-sticky settings {#task-3}

Checkpoint: `LAB15-CP03`

Purpose and operational relevance: Configure and independently inspect the control-plane and data-path properties needed to configure scale-out and slot-sticky settings. The change is unsafe when a setting becomes production-wide when it is updated without the deployment-slot setting flag. This checkpoint therefore proves: The plan worker count matches the target and the designated application setting remains sticky to the slot.
...
Expected state: The plan worker count matches the target and the designated application setting remains sticky to the slot.

Representative redacted output:

```text
The plan worker count matches the target and the designated application setting remains sticky to the slot.
```

Positive validation:

```powershell
$sku = az appservice plan show --resource-group $ResourceGroupName --name $plan --query sku.name --output tsv
if ($sku -ne 'S2') { throw "App Service plan SKU is $sku." }
```

Expected positive result: The plan worker count matches the target and the designated application setting remains sticky to the slot.
...
Expected negative result: LAB15-CP03 negative boundary: The undesired state is absent; the negative assertion must not report: Autoscale maximum is reported maximum.
...
- Positive assertion proving: The plan worker count matches the target and the designated application setting remains sticky to the slot.
- Negative assertion proving absence of: Autoscale maximum is reported maximum.

Common failure and safe retry: A setting becomes production-wide when it is updated without the deployment-slot setting flag. Retry LAB15-CP03 for configure scale-out and slot-sticky settings only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After LAB15-CP04, LAB15-CP05, remove the exact manifest IDs created or configured by LAB15-CP03 for configure scale-out and slot-sticky settings; its residual probe must then return zero active IDs.

## Task 4 — Swap staging into production with a recoverable path {#task-4}

Checkpoint: `LAB15-CP04`

Purpose and operational relevance: Exercise the deterministic operational or denied path needed to swap staging into production with a recoverable path, then retain comparable before-and-after results. The change is unsafe when a failed health check or long-running deployment can make a swap time out while both slots still exist. This checkpoint therefore proves: The swap completes, both slots remain provisioned, and the sticky value stays with its intended slot.
...
Expected state: The swap completes, both slots remain provisioned, and the sticky value stays with its intended slot.

Representative redacted output:

```text
The swap completes, both slots remain provisioned, and the sticky value stays with its intended slot.
```

Positive validation:

```powershell
$appId = az webapp show --resource-group $ResourceGroupName --name $app --query id --output tsv
$sticky = az resource show --ids "$appId/config/slotConfigNames" --api-version 2023-12-01 --query "contains(properties.appSettingNames, 'LAB_STAGE')" --output tsv
if ($sticky -ne 'true') { throw 'LAB_STAGE is not slot-sticky.' }
```

Expected positive result: The swap completes, both slots remain provisioned, and the sticky value stays with its intended slot.

Negative validation:

```powershell
$failedSlots = az webapp deployment slot list --resource-group $ResourceGroupName --name $app --query "[?state!='Running'] | length(@)" --output tsv
if ([int]$failedSlots -ne 0) { throw 'A deployment slot is not running.' }
```

Expected negative result: LAB15-CP04 negative boundary: The undesired state is absent; the negative assertion must not report: A deployment slot is not running.
...
- Positive assertion proving: The swap completes, both slots remain provisioned, and the sticky value stays with its intended slot.
- Negative assertion proving absence of: A deployment slot is not running.

Common failure and safe retry: A failed health check or long-running deployment can make a swap time out while both slots still exist. Retry LAB15-CP04 for swap staging into production with a recoverable path only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After LAB15-CP05, remove the exact manifest IDs created or configured by LAB15-CP04 for swap staging into production with a recoverable path; its residual probe must then return zero active IDs.

## Task 5 — Reconcile worker count, slot state, and configuration scope {#task-5}

Checkpoint: `LAB15-CP05`

Purpose and operational relevance: Reconcile live service properties, returned IDs, and dependency-aware removal needed to reconcile worker count, slot state, and configuration scope before handoff. The change is unsafe when reading only app settings from one slot can conceal a value that incorrectly moved during swap. This checkpoint therefore proves: Plan capacity, production/slot hostnames, and slotConfigNames prove the intended scaled release boundary.
...
Expected state: Plan capacity, production/slot hostnames, and slotConfigNames prove the intended scaled release boundary.

Representative redacted output:

```text
Plan capacity, production/slot hostnames, and slotConfigNames prove the intended scaled release boundary.
```

Positive validation:

```powershell
$planState = az appservice plan show --resource-group $ResourceGroupName --name $plan --query status --output tsv
if ($planState -ne 'Ready') { throw "Plan status is $planState." }
```

Expected positive result: Plan capacity, production/slot hostnames, and slotConfigNames prove the intended scaled release boundary.

Negative validation:

```powershell
$disabled = az monitor autoscale show --resource-group $ResourceGroupName --name $autoscale --query enabled --output tsv
if ($disabled -ne 'true') { throw 'Autoscale is disabled.' }
```

Expected negative result: LAB15-CP05 negative boundary: The undesired state is absent; the negative assertion must not report: Autoscale is disabled.
...
- Positive assertion proving: Plan capacity, production/slot hostnames, and slotConfigNames prove the intended scaled release boundary.
- Negative assertion proving absence of: Autoscale is disabled.

Common failure and safe retry: Reading only app settings from one slot can conceal a value that incorrectly moved during swap. Retry LAB15-CP05 for reconcile worker count, slot state, and configuration scope only after its exact recorded ID and current property are read. If the ID exists, reissue only the idempotent update; if it is absent, repeat only the owning create command, then rerun both assertions.

Cleanup dependency: After all service validation and evidence capture, remove the exact manifest IDs created or configured by LAB15-CP05 for reconcile worker count, slot state, and configuration scope; its residual probe must then return zero active IDs.
...
```powershell
./scripts/cli/Validate.ps1 -RunId 'az104l15-01' -Mode Deployment
az resource list --resource-group 'rg-az104-l15-az104l15-01' --query '[].{name:name,type:type}' --output table
```
...
```powershell
$appId = az webapp show --resource-group $ResourceGroupName --name $app --query id --output tsv
az resource update --ids "$appId/config/slotConfigNames" --api-version 2023-12-01 --set properties.appSettingNames=[] --output none
```

Capture the failed state with a read-only query:

```powershell
az resource show --ids "$appId/config/slotConfigNames" --api-version 2023-12-01 --query properties.appSettingNames --output json
```

Repair only the injected setting, then repeat the same query:

```powershell
az resource update --ids "$appId/config/slotConfigNames" --api-version 2023-12-01 --set properties.appSettingNames=['LAB_STAGE'] --output none
az resource show --ids "$appId/config/slotConfigNames" --api-version 2023-12-01 --query properties.appSettingNames --output json
```

Expected symptom: The value follows content and production receives the staging value.
...
Create a release runbook with warm-up, smoke test, swap, health validation, and immediate rollback criteria.
...
```powershell
./scripts/cli/Cleanup.ps1 -RunId 'az104l15-01'
./scripts/cli/Cleanup.ps1 -RunId 'az104l15-01' -Execute
./scripts/cli/Validate.ps1 -RunId 'az104l15-01' -Mode PostCleanup
$disabled = az monitor autoscale show --resource-group $ResourceGroupName --name $autoscale --query enabled --output tsv
if ($disabled -ne 'true') { throw 'Autoscale is disabled.' }
```
...
- [Create an App Service app with Azure CLI](https://learn.microsoft.com/en-us/azure/app-service/quickstart-cli)
- [Scale up an App Service plan](https://learn.microsoft.com/en-us/azure/app-service/manage-scale-up)
- [Set up staging environments with deployment slots](https://learn.microsoft.com/en-us/azure/app-service/deploy-staging-slots)
...
if ($state.labId -ne 'LAB-15' -or $state.runId -ne $RunId) { throw 'Manifest ownership does not match this lab and run ID.' }
$PSNativeCommandUseErrorActionPreference = $false
...
# CHECKPOINT LAB15-CP01 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB15-CP01' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB15-CP01' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab15-cp01.positive' -CheckpointId 'LAB15-CP01' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab15-cp01.negative' -CheckpointId 'LAB15-CP01' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
...
if ($registration -ne 'Registered') { throw 'Microsoft.Web is not registered.' }
    ...
    Add-ValidationCheck -Id 'lab15-cp01.positive' -CheckpointId 'LAB15-CP01' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $positivePassed -Command 'query each manifest-recorded object by exact ID and run the authored service probe' -Expected 'Checkpoint state is pass, recorded objects exist, and service state matches.' -Actual $positiveActual
...
if ([int]$supported -eq 0) { throw 'Linux S1 is unavailable in the selected region.' }
    ...
    Add-ValidationCheck -Id 'lab15-cp01.negative' -CheckpointId 'LAB15-CP01' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    ...
    Add-ValidationCheck -Id 'lab15-cp01.residual' -CheckpointId 'LAB15-CP01' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB15-CP01 END

# CHECKPOINT LAB15-CP02 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB15-CP02' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB15-CP02' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab15-cp02.positive' -CheckpointId 'LAB15-CP02' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab15-cp02.negative' -CheckpointId 'LAB15-CP02' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
    $positivePassed = $checkpointState.status -eq 'pass'
    $positiveActual = "checkpoint=$($checkpointState.status); managedObjects=$($checkpointObjects.Count)"
    foreach ($managedObject in $checkpointObjects) {
...
if ([int]$slotCount -ne 1) { throw 'The staging slot does not exist exactly once.' }
    ...
    Add-ValidationCheck -Id 'lab15-cp02.positive' -CheckpointId 'LAB15-CP02' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $positivePassed -Command 'query each manifest-recorded object by exact ID and run the authored service probe' -Expected 'Checkpoint state is pass, recorded objects exist, and service state matches.' -Actual $positiveActual
...
if ($stateValue -ne 'Running') { throw "Web app state is $stateValue." }
    ...
    $negativePassed = $unsafe.Count -eq 0 -and $negativeProbePassed
    $negativeActual = "unsafeObjects=$($unsafe.Count)" + "; $negativeProbeActual"
    Add-ValidationCheck -Id 'lab15-cp02.negative' -CheckpointId 'LAB15-CP02' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    $active = [System.Collections.Generic.List[string]]::new()
    foreach ($managedObject in $checkpointObjects) {
    ...
    Add-ValidationCheck -Id 'lab15-cp02.residual' -CheckpointId 'LAB15-CP02' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB15-CP02 END

# CHECKPOINT LAB15-CP03 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB15-CP03' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB15-CP03' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab15-cp03.positive' -CheckpointId 'LAB15-CP03' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab15-cp03.negative' -CheckpointId 'LAB15-CP03' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
    $positivePassed = $checkpointState.status -eq 'pass'
    $positiveActual = "checkpoint=$($checkpointState.status); managedObjects=$($checkpointObjects.Count)"
    foreach ($managedObject in $checkpointObjects) {
...
if ([int]$maximum -ne 3) { throw "Autoscale maximum is $maximum." }
    ...
    $negativePassed = $unsafe.Count -eq 0 -and $negativeProbePassed
    $negativeActual = "unsafeObjects=$($unsafe.Count)" + "; $negativeProbeActual"
    Add-ValidationCheck -Id 'lab15-cp03.negative' -CheckpointId 'LAB15-CP03' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    $active = [System.Collections.Generic.List[string]]::new()
    foreach ($managedObject in $checkpointObjects) {
    ...
    Add-ValidationCheck -Id 'lab15-cp03.residual' -CheckpointId 'LAB15-CP03' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB15-CP03 END

# CHECKPOINT LAB15-CP04 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB15-CP04' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB15-CP04' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab15-cp04.positive' -CheckpointId 'LAB15-CP04' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab15-cp04.negative' -CheckpointId 'LAB15-CP04' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
    $positivePassed = $checkpointState.status -eq 'pass'
    $positiveActual = "checkpoint=$($checkpointState.status); managedObjects=$($checkpointObjects.Count)"
    foreach ($managedObject in $checkpointObjects) {
...
if ($sticky -ne 'true') { throw 'LAB_STAGE is not slot-sticky.' }
    ...
    Add-ValidationCheck -Id 'lab15-cp04.positive' -CheckpointId 'LAB15-CP04' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $positivePassed -Command 'query each manifest-recorded object by exact ID and run the authored service probe' -Expected 'Checkpoint state is pass, recorded objects exist, and service state matches.' -Actual $positiveActual
...
if ([int]$failedSlots -ne 0) { throw 'A deployment slot is not running.' }
    ...
    $negativePassed = $unsafe.Count -eq 0 -and $negativeProbePassed
    $negativeActual = "unsafeObjects=$($unsafe.Count)" + "; $negativeProbeActual"
    Add-ValidationCheck -Id 'lab15-cp04.negative' -CheckpointId 'LAB15-CP04' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    $active = [System.Collections.Generic.List[string]]::new()
    foreach ($managedObject in $checkpointObjects) {
    ...
    Add-ValidationCheck -Id 'lab15-cp04.residual' -CheckpointId 'LAB15-CP04' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB15-CP04 END

# CHECKPOINT LAB15-CP05 BEGIN
$checkpointState = @($state.checkpointStates | Where-Object { $_.checkpointId -eq 'LAB15-CP05' })[0]
$checkpointObjects = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB15-CP05' })

if ($Mode -eq 'Deployment') {
    if ($checkpointState.status -eq 'skipped') {
        Add-ValidationCheck -Id 'lab15-cp05.positive' -CheckpointId 'LAB15-CP05' -Kind 'positive' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'The optional checkpoint is explicitly skipped when its documented input is absent.' -Actual 'optional gate absent'
        Add-ValidationCheck -Id 'lab15-cp05.negative' -CheckpointId 'LAB15-CP05' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $true -Skipped $true -Command 'optional gate evaluation' -Expected 'No mutation occurs for a skipped optional checkpoint.' -Actual 'optional gate absent'
    } else {
    $positivePassed = $checkpointState.status -eq 'pass'
    $positiveActual = "checkpoint=$($checkpointState.status); managedObjects=$($checkpointObjects.Count)"
    foreach ($managedObject in $checkpointObjects) {
...
if ($disabled -ne 'true') { throw 'Autoscale is disabled.' }
    ...
    $negativePassed = $unsafe.Count -eq 0 -and $negativeProbePassed
    $negativeActual = "unsafeObjects=$($unsafe.Count)" + "; $negativeProbeActual"
    Add-ValidationCheck -Id 'lab15-cp05.negative' -CheckpointId 'LAB15-CP05' -Kind 'negative' -Required ([bool]$checkpointState.required) -Passed $negativePassed -Command 'run the authored denied-state probe and compare manifest ownership' -Expected 'The service boundary and ownership checks both pass.' -Actual $negativeActual
    }
} else {
    $active = [System.Collections.Generic.List[string]]::new()
    foreach ($managedObject in $checkpointObjects) {
    ...
    Add-ValidationCheck -Id 'lab15-cp05.residual' -CheckpointId 'LAB15-CP05' -Kind 'residual' -Required ([bool]$checkpointState.required) -Passed ($active.Count -eq 0) -Command 'query every recorded object by exact ID after cleanup' -Expected 'No active manifest-managed object remains.' -Actual "active=$($active -join ',')"
}
# CHECKPOINT LAB15-CP05 END
...
function Write-CleanupRefusal {
    ...
    $refusal = @{
        schemaVersion = '1.0.0'; labId = 'LAB-15'; runId = $RunId
        generatedAt = (Get-Date).ToUniversalTime().ToString('o')
...
if ($state.labId -ne 'LAB-15' -or $state.runId -ne $RunId) {
...
foreach ($object in $active) {
    ...
    if ($method -eq 'manifest-id-and-tags' -and $object.kind -eq 'azure-resource') {
        ...
        $manifestTagsMatch =
            ...
            [string]$expectedTags['labId'] -eq '15' -and
...
# CHECKPOINT LAB15-CP05 BEGIN
$targets = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB15-CP05' -and $_.lifecycleStatus -eq 'active' })
foreach ($target in $targets) {
    $alreadyAbsent = $absentIds.Contains([string]$target.id)
    $actions.Add(@{ checkpointId = 'LAB15-CP05'; targetId = [string]$target.id; targetType = [string]$target.type; ownership = @{ method = [string]$target.ownership.method; verified = [bool]$verifiedOwnershipById[[string]$target.id] }; status = $(if ($alreadyAbsent) { 'skipped' } else { 'preview' }); message = $(if ($alreadyAbsent) { 'The exact manifest ID was already absent; idempotent cleanup recorded the safe end state.' } elseif ($Execute) { 'Pending removal through the dependency-safe ownership boundary.' } else { 'Would remove this exact manifest-recorded target.' }) })
}
$state.updatedAt = (Get-Date).ToUniversalTime().ToString('o')
$state | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Manifest -Encoding utf8
# CHECKPOINT LAB15-CP05 END
...
# CHECKPOINT LAB15-CP04 BEGIN
$targets = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB15-CP04' -and $_.lifecycleStatus -eq 'active' })
foreach ($target in $targets) {
    $alreadyAbsent = $absentIds.Contains([string]$target.id)
    $actions.Add(@{ checkpointId = 'LAB15-CP04'; targetId = [string]$target.id; targetType = [string]$target.type; ownership = @{ method = [string]$target.ownership.method; verified = [bool]$verifiedOwnershipById[[string]$target.id] }; status = $(if ($alreadyAbsent) { 'skipped' } else { 'preview' }); message = $(if ($alreadyAbsent) { 'The exact manifest ID was already absent; idempotent cleanup recorded the safe end state.' } elseif ($Execute) { 'Pending removal through the dependency-safe ownership boundary.' } else { 'Would remove this exact manifest-recorded target.' }) })
}
$state.updatedAt = (Get-Date).ToUniversalTime().ToString('o')
$state | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Manifest -Encoding utf8
# CHECKPOINT LAB15-CP04 END
...
# CHECKPOINT LAB15-CP02 BEGIN
$targets = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB15-CP02' -and $_.lifecycleStatus -eq 'active' })
foreach ($target in $targets) {
    $alreadyAbsent = $absentIds.Contains([string]$target.id)
    $actions.Add(@{ checkpointId = 'LAB15-CP02'; targetId = [string]$target.id; targetType = [string]$target.type; ownership = @{ method = [string]$target.ownership.method; verified = [bool]$verifiedOwnershipById[[string]$target.id] }; status = $(if ($alreadyAbsent) { 'skipped' } else { 'preview' }); message = $(if ($alreadyAbsent) { 'The exact manifest ID was already absent; idempotent cleanup recorded the safe end state.' } elseif ($Execute) { 'Pending removal through the dependency-safe ownership boundary.' } else { 'Would remove this exact manifest-recorded target.' }) })
}
$state.updatedAt = (Get-Date).ToUniversalTime().ToString('o')
$state | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Manifest -Encoding utf8
# CHECKPOINT LAB15-CP02 END
...
# CHECKPOINT LAB15-CP01 BEGIN
$targets = @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB15-CP01' -and $_.lifecycleStatus -eq 'active' })
foreach ($target in $targets) {
    $alreadyAbsent = $absentIds.Contains([string]$target.id)
    $actions.Add(@{ checkpointId = 'LAB15-CP01'; targetId = [string]$target.id; targetType = [string]$target.type; ownership = @{ method = [string]$target.ownership.method; verified = [bool]$verifiedOwnershipById[[string]$target.id] }; status = $(if ($alreadyAbsent) { 'skipped' } else { 'preview' }); message = $(if ($alreadyAbsent) { 'The exact manifest ID was already absent; idempotent cleanup recorded the safe end state.' } elseif ($Execute) { 'Pending removal through the dependency-safe ownership boundary.' } else { 'Would remove this exact manifest-recorded target.' }) })
}
$state.updatedAt = (Get-Date).ToUniversalTime().ToString('o')
$state | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Manifest -Encoding utf8
# CHECKPOINT LAB15-CP01 END
...
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB15-CP01' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp01.residual'; command = 'query manifest-recorded IDs for LAB15-CP01'; expected = 'zero active objects for LAB15-CP01'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB15-CP02' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp02.residual'; command = 'query manifest-recorded IDs for LAB15-CP02'; expected = 'zero active objects for LAB15-CP02'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB15-CP03' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp03.residual'; command = 'query manifest-recorded IDs for LAB15-CP03'; expected = 'zero active objects for LAB15-CP03'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB15-CP04' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp04.residual'; command = 'query manifest-recorded IDs for LAB15-CP04'; expected = 'zero active objects for LAB15-CP04'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
$checkpointRemaining = @($remaining | Where-Object { $_ -in @($state.managedObjects | Where-Object { $_.checkpointId -eq 'LAB15-CP05' } | ForEach-Object id) }); $residualChecks.Add(@{ id = 'cp05.residual'; command = 'query manifest-recorded IDs for LAB15-CP05'; expected = 'zero active objects for LAB15-CP05'; actual = "active=$($checkpointRemaining.Count)"; status = $(if ($Execute -and $checkpointRemaining.Count -eq 0) { 'pass' } elseif (-not $Execute) { 'skipped' } else { 'fail' }) })
