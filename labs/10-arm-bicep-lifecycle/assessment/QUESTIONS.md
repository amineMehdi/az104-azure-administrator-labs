...
## 1. Which statement best captures the key design principle for interpret, modify, deploy, export, and decompile arm and bicep? (`LAB10-Q01`)
...
## 2. You must begin the hands-on path for 'Interpret an Azure Resource Manager template or a Bicep file'. Which action matches the reviewed lab sequence? (`LAB10-Q02`)
...
## 3. Which authorization statement is appropriate before creating resource group, Bicep deployment, StorageV2 account in this lab? (`LAB10-Q03`)
...
## 4. The setup command for Lab 10 is run without its execution switch. What should happen? (`LAB10-Q04`)
...
## 5. Which validation evidence most directly proves the baseline resource boundary for Lab 10? (`LAB10-Q05`)

- A. Query the exact recorded scope and verify the intended resource/object types, including Microsoft.Resources/deployments, Microsoft.Storage/storageAccounts.
...
## 6. A learner reaches the externally gated checkpoint in Lab 10. What is the safest response? (`LAB10-Q06`)
...
## 7. Which command-side evidence best supports the first completed checkpoint in Lab 10? (`LAB10-Q07`)
...
- C. Run the independent validator and retain redacted structured output for the exact recorded scope after: Build and lint the supplied Bicep file and inspect its generated ARM JSON.
- D. Use the setup process exit code alone without querying resource state.

## 8. During cleanup of Lab 10, several similarly named resources exist. Which targeting method is correct? (`LAB10-Q08`)
...
## 9. Setup completed for Lab 10, but one required state check fails. What is the best break/fix approach? (`LAB10-Q09`)
...
## 10. Which completion claim is valid immediately after this repository's offline tests pass for Lab 10? (`LAB10-Q10`)
