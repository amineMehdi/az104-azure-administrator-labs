# Lab 14: Deploy and scale Azure Container Apps
...
|---|---|
| `CP-CONTAINERS-03` | Provision a container by using Azure Container Apps |
| `CP-CONTAINERS-04` | Manage sizing and scaling for containers, including Azure Container Instances and Azure Container Apps |
...
![Lab 14 architecture](diagrams/architecture.svg)
...
The key design idea is: **Container Apps revisions are immutable deployment snapshots, while replicas scale within an active revision according to minimum, maximum, and event rules.**
...
- Resource providers observed by preflight: `Microsoft.App`, `Microsoft.OperationalInsights`
...
- Choose a unique run ID matching `^[a-z0-9-]+$`, such as `az104l14-01`.
...
echo "  resources: resource group, Log Analytics workspace, Container Apps environment, container app"
...
jq -n   --arg labId "LAB-14" --arg runId "$RUN_ID" --arg tenantId "$TENANT_ID"   --arg subscriptionId "$SUBSCRIPTION_ID" --arg location "$LOCATION" --arg rgName "$RG"   --arg createdAt "$(date -u +%Y-%m-%dT%H:%M:%SZ)"   '{labId:$labId,runId:$runId,tenantId:$tenantId,subscriptionId:$subscriptionId,location:$location,createdAt:$createdAt,status:"recorded-before-mutation",resourceGroup:{name:$rgName,id:null},resources:[],external:{}}' >"$MANIFEST"

az group create --subscription "$SUBSCRIPTION_ID" --name "$RG" --location "$LOCATION"   --tags purpose=az104-lab labId=14 runId="$RUN_ID" expiresOn="$EXPIRES_ON" --output none
...
if [[ "$purpose" == "az104-lab" && "$lab_id" == "14" && "$run_id" == "$RUN_ID" ]]; then
...
jq -n --arg labId "LAB-14" --arg runId "$RUN_ID" --arg generatedAt "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg result "$result" --argjson checks "$checks"   '{labId:$labId,runId:$runId,generatedAt:$generatedAt,result:$result,checks:$checks}' >"$REPORT"
cat "$REPORT"
[[ "$result" != fail ]]
```
...
RG="$(jq -r '.resourceGroup.name' "$MANIFEST")"
RG_ID="$(jq -r '.resourceGroup.id' "$MANIFEST")"
echo "Cleanup preview for LAB-14:"
...
[[ "${actual,,}" == "${RG_ID,,}" && "$purpose" == az104-lab && "$lab_id" == 14 && "$run_id" == "$RUN_ID" ]] || {
...
```sh
./scripts/cli/setup.sh --subscription-id <subscription-id> --location <region> --run-id az104l14-01
```
...
```sh
./scripts/cli/setup.sh --subscription-id <subscription-id> --location <region> --run-id az104l14-01 --execute
```
...
### Checkpoint 1: Verify the containerapp Azure CLI extension and required provider registrations
...
### Checkpoint 2: Create a managed environment and deploy a sample app with external ingress
...
### Checkpoint 3: Create a new revision by changing an environment variable and split traffic deliberately
...
### Checkpoint 4: Configure bounded HTTP scaling and compare replicas, revisions, and provisioning state
...
```sh
./scripts/cli/validate.sh --subscription-id <subscription-id> --run-id az104l14-01
```

Inspect `.state/az104l14-01/validation.json`. A `pass` applies only to checks that could be executed. A gated or asynchronous path must remain `warning` or `skipped` until its evidence exists.
...
```sh
./scripts/cli/cleanup.sh --subscription-id <subscription-id> --run-id az104l14-01
```

After verifying every printed target belongs to this run:

```sh
./scripts/cli/cleanup.sh --subscription-id <subscription-id> --run-id az104l14-01 --execute
```
...
- [https://learn.microsoft.com/en-us/azure/container-apps/get-started](https://learn.microsoft.com/en-us/azure/container-apps/get-started)
- [https://learn.microsoft.com/en-us/azure/container-apps/revisions](https://learn.microsoft.com/en-us/azure/container-apps/revisions)
...
- [https://learn.microsoft.com/en-us/azure/container-apps/ingress-overview](https://learn.microsoft.com/en-us/azure/container-apps/ingress-overview)
